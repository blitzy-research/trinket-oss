...
module.exports = {
  ...
  email : async function(request, h) {
    ...
    // recaptcha.verify resolves a single result object - it is promise-native, so no bridge is
    // needed - and with no secret key configured, the default and always the case under test, it
    // hands back an already-resolved promise. It never reports a transport error: see
    // lib/util/recaptcha.js for the fate that takes instead.
    ...
    if (result.success) {
      ...
      return mailer.send(request.payload.email, subject, options)
        ...
        .catch(function(err) {
          ...
          // 200 for a mail-send failure is exactly the substitution R-4 and TR2 forbid. The
          // second arm is `h.abandon`, hapi's own no-response outcome - see
          // lib/http/responseContract.js.
  ...
  snapshot : async function(request, h) {
    if (request.payload.snapshotData) {
      ...
      // Async conversion: FileUtil.uploadSnapshotFromBuffer is promise-native - it returns
      // FileUtil._upload's own promise - so this awaits it directly, with neither
      // util.promisify nor the hand-written bridge this handler used to carry. The next link
      // in the chain ignores its argument, so resolving with the SDK's response where the
      // base commit's bridge resolved with `undefined` is unobservable.
  ...
  downloadPostedZip : async function(request, h) {
    ...
    return Promise.all(assetPromises)
      ...
      .catch(function(err) {
        // RESOURCE OWNERSHIP (review finding F2), before the cleanup and the response that follow it.
        // Every path into this catch - an archive 'error', a non-ENOENT 'warning', an
        // outputWriteStream 'error', a finalize() rejection, a JSON.parse throw, an asset append
        // failure - leaves an archiver alive and an OPEN descriptor on `zipFile`, because both are
        // allocated at the head of this handler and piped together before anything can fail. Aborting
        // the archive releases its queue and pending sources; destroying the write stream releases the
        // descriptor, and it must happen before the unlink so the file is not unlinked from under an
        // open handle.
        //
        // Idempotent and error-only: it runs on no successful path, `abort()`'s own raise is swallowed
        // (archiver 7 raises when the archive was never finalized or is already aborted, and that says
        // nothing about why the request failed), and `destroyed` is checked so a stream that already
        // closed is left alone. Nothing about the response moves: the SAME `err.message` is handed to
        // badImplementation, so the 500's scrubbed body and its status are untouched, and no member
        // order, byte count or Content-Disposition on the success path is affected.
        //
        // This is deliberately NOT applied to downloadZip below, whose archive error listener is
        // misspelled `'err'`: that handler's documented fate is a rejection nobody consumes and a
        // request that never answers, and touching its resources would be the back-door repair
        // docs/PRESERVED-QUIRKS.md section 3.20 forbids.
 ...
 * up by accident. See review finding F2 and docs/PRESERVED-QUIRKS.md sections 3.20 and 3.41.
 *
