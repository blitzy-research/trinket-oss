 ...
 * next, and the SEC-4 / SV-02 remediation replaced that write-back with a
 * request-local `target` - see the comment at that site and
 * docs/PRESERVED-QUIRKS.md 4.4.
 *
...
function createResponders(spec) {
  ...
  function reject(request, h, json, err, responseType) {
    ...
    // (docs/PRESERVED-QUIRKS.md section 15.6). lib/http/validation.js hands this responder
    // `request.payload`, so the failure payload of a failed signup or password change carries
    // the SUBMITTED PASSWORD.
    //   - The LOG copy IS scrubbed. `redactSecrets()` is the F-16/S-2 remediation, and it is the
    //     cheapest of them to justify because it is not a behavior change at all: it inspects a
    //     COPY, so nothing observable moves.
    //   - The WIRE copy is NOT scrubbed, and must not be. `json` itself is flashed as 'failure'
    //     and re-rendered into the form below verbatim - the base commit's behavior - so
    //     redacting it there would change a rendered page, which R-4 forbids.
    if (json) {
      log.info(util.inspect(redactSecrets(json)) + " " + err);
    }

    if (responseType === 'html' && fail.redirect) {
      // SECURITY REMEDIATION (review finding SEC-4 / SV-02, CWE-601 + CWE-362), recorded in
      ...
      // other visitor, until restart - reused the attacker's value. Consequence (b)
      // is a CROSS-REQUEST leak, which is why this is a correctness fix as well as
      // a security one and why it stands independently of the confinement decision.
      //
      ...
      // before and after, which is why test/baseline/replay.js reports 0
      // differences with this in place.
