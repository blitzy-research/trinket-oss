...
function copy(user, cb) {
  ...
  // instead of the body, never after it. A trailing .catch would instead divert the deliberately
  // unterminated inner chain below into cb. The duplicate-key error this save exists to catch must
  // reach cb unwrapped, so the handler forwards it as-is.
  course.save().then(function(doc) {
    ...
    // Async conversion: lib/models/lesson.js#copy is promise-native, so no bridge is needed. Its
    // promise stays permanently pending on an upstream failure - the fate documented there - which
    // this .then propagates unchanged.
    ...
    Promise.all(lessonPromises)
      ...
      .then(function(ids) {
        ...
        // Deliberately unterminated: a rejection from Lesson.findById or lesson.copy never reaches cb.
        // No `return`, and the two-argument .then keeps a throw from cb out of cb. The logging call
        // below is retained.
        course.save().then(function(doc) {
          return cb(null, doc);
        }, function(err) {
