 ...
 *   isMobile(request)
 *   aceOff(request)
 *   pending                           // { forever, hang, rejectOrHang } - the no-response fate
 *
 ...
 * responder mutates them. The base commit's reject() DID: it wrote its
 * interpolated URL back into `fail`, which leaked one request's value into the
 * next, and the SEC-4 remediation replaced that write-back with a request-local
 * `target` - see the comment at that site and docs/PRESERVED-QUIRKS.md 4.4.
 *
...
function createResponders(spec) {
  ...
  function respond(request, h, json, responseType) {
    ...
    if (responseType === 'html' && success.html && !JSON_EXT.test(request.url.pathname)) {
      ...
      if (typeof(success.html) === 'string') {
        ...
        //
        // The payload is redacted for the same reason as in reject() (F-16/S-2): a
        // success payload can carry a populated `_owner`, which is where the bcrypt hash
        // SEC-13 removed from the wire lives. The 500 and its scrubbed message are
        // unchanged - only what lands in the log moves.
  ...
  function reject(request, h, json, err, responseType) {
    ...
    // itself is flashed and returned untouched below, so the wire behavior and the
    // re-rendered form are byte-identical to the base commit. An Error handed here as
    // `json` is passed through unchanged, so a rejection still logs what it always did.
    if (json) {
 ...
 * reproduced instead: a promise that never settles leaves the request in exactly the state the unsettled
 * deferred left it in. Measured: no response, the server stays listening, every other route is served
 * normally, and nothing is logged - which also matches the base commit, where an ignored callback error
 * produced no log line. No route sets a server timeout (`route.settings.timeout` is `{ server: false }`
 * on all 233 rows), so nothing converts the wait into a 503.
 *
 * These fates live here rather than in a module of their own because "what a route answers" is this
 * module's subject and because `rejectOrHang` wraps the very `reject` responder created above. Which
 * branches use them is a MEASURED question, not an inferred one: a branch qualifies only once it has been
 * reproduced against a verbatim replica of the base-commit wrapper on @hapi/hapi 20.3.0 and observed to
 * answer nothing. The synthetic responder was far more forgiving than it looks - `request.success({ x:
 ...
 * can turn a later `respond()` into a response: the value is inert, a FRESH promise per call so a hung
 * request's continuation is retained by that request alone, and every use is one greppable line at the
 * call site. `hang()` reproduces the HTTP fate only, NOT the process termination that accompanied some of
 * those branches (a Mongoose callback invoked with the document absent, dereferenced, and re-thrown
 * through `immediate()` with no `uncaughtException` handler anywhere in this tree). That narrowing is an
 * explicit R-6 adjudication, recorded in docs/PRESERVED-QUIRKS.md: killing a shared server process is not
 * an implementable route behaviour, whereas the pending request is, and the pending request is the part a
 * client can observe.
 *
 *   var Pending = require('../http/responseContract').pending;
 *   ...
 *   catch (saveError) {
 ...
 *   }
 *
 * Always `return` it. Calling it without returning resolves the handler with `undefined` and produces the
 * very 500 this exists to avoid.
 *
...
/**
 * Alias of forever(). Both spellings are in use across lib/controllers/, and both return a fresh
 ...
 * lib/http/errorMap.js, which answers a scrubbed 500 those branches never carried.
 *
...
module.exports = {
  ...
  // The `pending` namespace lives here rather than in a seventh lib/http module so the
  // extracted-module count stays at the six the plan specifies; all call sites reach it
  // through `require('../http/responseContract').pending`.
  pending          : {
    forever      : forever,
    hang         : hang,
