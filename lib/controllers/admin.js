var config        = require('config'),
    ...
    // SECURITY REMEDIATION (review finding M6, CWE-200/CWE-522). The single centralized credential
    // scrub applied by every surface that ships a JSON-cloned User document. See lib/util/credentials.js
    // and docs/PRESERVED-QUIRKS.md section 4.14.
...
module.exports = {
  ...
  uploadUsers : async function(request, h) {
    ...
    try {
      ...
      //
      // R-6, and measured rather than assumed - see lib/http/responseContract.js#rejectOrHang for
  ...
  updateUser : async function(request, h) {
    ...
    try {
      ...
      // rejection and this branch answered NOTHING. See lib/http/responseContract.js#rejectOrHang.
      return Pending.rejectOrHang(h, err);
    }
  ...
  grantRole : async function(request, h) {
    ...
    return user.grant(request.payload.role, "site")
      ...
      .then(function(user) {
        // SEC-13, extended by review finding M6 - see docs/PRESERVED-QUIRKS.md section 4.14. The
        // `JSON.parse(JSON.stringify(user))` round-trip is the baseline payload shape and is kept as
        // the payload shape: it clones the whole User document rather than projecting `publicSpec`,
        // which is why this response disclosed ANOTHER user's bcrypt hash to the admin caller.
        //
        // The first remediation deleted the top-level `password` key alone, which was not enough.
        ...
        var payload = Credentials.redact(JSON.parse(JSON.stringify(user)));
      ...
      .catch(function(err) {
        // R-6 - see lib/http/responseContract.js#rejectOrHang. At the base commit this chain was
...
async function userSearch(q) {
  ...
  // SEC-13, extended by review finding M6 - see docs/PRESERVED-QUIRKS.md section 4.14. This
  // round-trip is the baseline shape and stays the shape, but it flattens the User document to a plain
  ...
  // That is how the owner's bcrypt hash arrived in the rendered admin page, which dumps this object
  // wholesale through `{{ data | json("pretty") | safe }}` in lib/views/admin/includes/users.html - a
  // frozen template, so the credentials are removed here at the source instead. The template's named
  // reads are `_id`, `avatar`, `coursesOwned`, `email`, `name`, `roles`, `trinketsOwned` and
  // `username`; no credential is among them, and `roleSearch` below already answers clean because it
  // hands the responder real documents.
  //
