...
module.exports = {
  createCourse : async function(request, h) {
    ...
    return course.addUser(request.user, ["course-owner"])
      .then(function() {
        // PRESERVED QUIRK (docs/PRESERVED-QUIRKS.md section 4.14). `Course.publicSpec` whitelists
        // `_owner`, and `setOwner` assigns the populated User DOCUMENT to it, so the success
        // responder's `serialize()` reaches its nested-object branch. That branch tests
        // `hasOwnProperty('serialize')`, but mongoose installs `serialize` on the document PROTOTYPE,
        // so the test fails and the owner is JSON-cloned whole - shipping the owner's bcrypt hash in
        // this body and, through the server.inject in lib/controllers/courses.js#create, in
        // POST /courses too. That is the base commit's payload and it is reproduced unchanged: R-4
        // freezes the shape, R-6 makes the base commit the tie-breaker, and the condition is
        // catalogued rather than fixed on this branch.
        //
