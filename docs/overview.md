...
Seven links appear below this line, and each one was resolved live and returns HTTP 200. **Five** of the seven are component-repository links backed by a `_source` field in those same `.bower.json` files - skulpt-dist, marked, jq-console, traqball.js and Detectizr - and **all five match their `_source`**. The remaining two, to `CONTRIBUTING.md` and to the issue tracker, are not component repositories and have no `.bower.json` to match. (An earlier revision of this line claimed all seven were `_source`-checked, which over-counted what was checkable.) None of the seven were introduced or changed by the Node 22 modernization.
...
3D graphics environment with VPython bindings. The GlowScript version is selected by configuration rather than fixed in code: `lib/views/embed/glowscript-config.html` publishes a map of eleven selectable versions (`1.1` through `3.2`) and `config/default.yaml` picks the default with `app.glowscript.defaultVersion`, which ships as `3.2`, served from `public/components/vpython-glowscript/` - whose hydrated `.bower.json` records `_release` **`3.1.0`** (its `package.json` says `2.9.0`). An earlier revision of this line published a Trinket build of `3.2.2`, which is the figure `COMPONENTS.md` catalogues as one of the three rows that disagree with the release asset; the `_release` reading is the authoritative one, and the variance note above covers only the `v`-prefix class of difference, so this row is called out explicitly here. The separate `glowscript-blocks` type - not the Blockly `/embed/blocks` above - is configured through `app.glowscript.blocksVersion`, which ships as `2.7` - Trinket build `2.7.5`, served from `public/components/glowscript/` - against its own seven-version map in `lib/views/embed/glowscript-blocks-config.html`. Both templates and both configuration values are unchanged by the Node 22 modernization.
