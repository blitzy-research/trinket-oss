...
function encryptPassword(next) {
  ...
  // bcrypt 6.0.0 returns a promise when no callback is supplied. This stays a next-style
  // mongoose middleware (arity 1, returns undefined) so kareem keeps treating it as one;
  // SALT_WORK_FACTOR is frozen because it is encoded in every persisted hash.
  ...
  bcrypt.genSalt(SALT_WORK_FACTOR)
    ...
    .catch(next); // hands next() the identical error object the callback form received
}
...
function exists(user, cb) {
  ...
  return this.model.find({ $or : or })
    ...
    .catch(cb); // hands cb the identical mongoose error object as its first argument
}
...
function findAdminList(page, cb) {
  var limit = 100;
  var skip  = page * limit;
  // No `return` here: baseline resolves to undefined, and the two-argument .then keeps the
  // success and failure paths disjoint so a throw from cb is not fed back into cb.
  // See docs/PRESERVED-QUIRKS.md.
