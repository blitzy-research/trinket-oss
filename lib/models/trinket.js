...
function hashify() {
  ...
  if (!this.shortCode) {
    // The truncation length here MUST match the one `verifyShortCode` derives
    // below: that function re-derives sha1(seed + timestamp) and discards the
    // document's shortCode when the two differ. It takes ten characters, so a
    // twelve-character generated value could never equal it and the rejection
    // path was unreachable-by-construction for every server-generated code.
    // The file disagreed with itself (12 here, 10 at `verifyShortCode`), so
    // there is no single baseline value to preserve; ten is the length the
    // comparison and the model's own test assert. Lookups resolve through
    // `alternateIds: ['shortCode']` against the STORED value, so previously
    // stored twelve-character codes keep resolving unchanged.
    this.shortCode = crypto.createHash('sha1').update(seed + Date.now()).digest('hex').substring(0, 10);
  }
}
...
function verifyShortCode(timestamp) {
  ...
  if (shortCode !== this.shortCode) {
    // `delete this.shortCode` was inert here and silently let the rejected
    // value through: `shortCode` is a schema path, so its accessor lives on the
    // document prototype and the document holds no own property of that name
    // for `delete` to remove. The client's value therefore survived the check
    // and was persisted, handing the caller control of the public alternate id
    // (`alternateIds: ['shortCode']`). Assigning through the path clears it for
    // real - it routes through the schema setter on a document, and leaves a
    // falsy value even if `this` is ever a plain object - so `hashify`'s
    // `if (!this.shortCode)` mints a server-generated code on the pre-save
    // hook, which is what an unverifiable client value must fall back to.
...
function findAndUpdateMetrics(id, metric, meta) {
  ...
  // The driver is given the callback rather than being awaited as a query.
  //
  // Two things force that. The four-argument callback form is the contract this
  // method's own test exercises (test/lib/models/trinket.js:145-151 stubs
  // `function (id, update, options, cb)`), and the three-argument promise form
  // called it with no `cb`, so the stub threw `cb is not a function`. And the
  // promise this method returns must carry `.done`, which the two cases call
  // (:187, :202) and which a native promise does not have. A `q` deferred
  // supplies it: `q` is already a declared dependency with live consumers
  // (lib/workers/exports.js, test/helpers/mail.js), and a chained `q` promise
  // exposes `.then`, `.catch`, `.done`, `.spread` and `.fail`, so every
  // production consumer (lib/controllers/trinket.js:590, 630, 693, 805, 1080 -
  // `.then`, one of them inside a `.spread` pair) keeps working unchanged.
  var deferred = Q.defer();

  this.model.findByIdAndUpdate(id, update, options, function(err, trinket) {
    if (err) {
      return deferred.reject(err);
    }

    // The try/catch reproduces the rejection the removed `.then` produced, and
    // is not defensive decoration: a null `trinket` makes `trinket._id` throw,
    // which used to reject this promise and answer 500 through the caller's
    // `.catch`. Inside a driver callback the same throw would instead escape as
    // an uncaught exception - nothing in this application installs an
    // 'uncaughtException' handler - so it is caught and turned back into the
    // rejection the callers already handle.
    //
    // The body is the removed `.then` callback's, in its order: construct the
    // interaction, issue the save without awaiting it, then settle with the
    // trinket. `interaction.save()` stays fire-and-forget in the promise form,
    // so an asynchronous save failure still surfaces exactly as it did before -
    ...
    try {
      ...
      deferred.resolve(trinket);
    } catch (interactionErr) {
      deferred.reject(interactionErr);
    }
  });
