...
// Serializes a flat field map onto the wire exactly as the retired HTTP client's `form` option did
// through qs.stringify. Three properties are reproduced, and all three are load-bearing:
...
//   3. every surviving value is percent-encoded with encodeURIComponent, so a space becomes %20 rather
//      than +, and the request keeps the bare `application/x-www-form-urlencoded` media type with no
//      charset parameter appended to it.
// Property 2 is the one this application actually exercises: config/default.yaml declares
// app.auth.google.clientSecret, .clientID and .callbackURL with NO value, node-config resolves a valueless
// YAML key to `null` rather than to `undefined`, and the base commit put `client_secret=&redirect_uri=` on
// the wire for the absent pair. That is also why a URLSearchParams body cannot stand in here: it
// serializes `null` and `undefined` as the literal strings "null" and "undefined", encodes a space as +,
// and sets the Content-Type to 'application/x-www-form-urlencoded;charset=UTF-8'.
// See docs/PRESERVED-QUIRKS.md section 3.37.
// Property 3 is where `encodeURIComponent` alone is NOT enough, and review finding
// F-09 reported the gap: qs's encoder percent-encodes everything except
// `A-Za-z0-9`, `-`, `.`, `_` and `~`, while `encodeURIComponent` additionally leaves
// `!`, `'`, `(`, `)` and `*` raw. The delta is exactly those five characters, so they
// are post-escaped here to the same uppercase-hex form qs emits (`%21 %27 %28 %29 %2A`).
// Everything else `encodeURIComponent` produces is already byte-identical to qs,
// `~` included - both leave it raw - and a space still becomes `%20`, never `+`.
// This matters because an authorization code, a client secret and a callback URL are
// all provider-controlled strings: a code containing `(` would otherwise have gone to
// Google in a different form than the retired client sent it.
...
module.exports = {
  ...
  google : async function(request, h) {
    ...
    // PRESERVED QUIRK (docs/PRESERVED-QUIRKS.md section 4.4). The value is persisted verbatim, and
    // googleCallback below feeds it straight into the `redirectTo` it hands the responder;
    // GET /auth/google/callback declares `success : { redirect : '{redirectTo}' }`
    // (config/routes.js:L537-L539), so it becomes a Location header. Nothing constrains the
    // destination to this application's own origin - the base commit's behavior, reproduced under
    // R-4 and R-6 rather than repaired.
  ...
  googleCallback : async function(request, h) {
    ...
    try {
      ...
      // The persisted value is consumed unfiltered - see the note where `google` above stores it.
      // '/home' answers only when nothing was persisted at all.
