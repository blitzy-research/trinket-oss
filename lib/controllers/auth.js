...
// a body of `null` parses to null. Those two values are the ones the
// `!body.access_token` and `!profile.email` guards below used to dereference
// unguarded; both guards now test the body itself first, so either value
// rejects the chain instead of throwing out of the callback. The approved
// deviation that decides this is recorded at the token guard.
...
// with no parsed body, which is one of the three shapes the token guard below
// refuses.
  ...
  googleCallback : function(request, h) {
    ...
    return new Promise(function(resolve, reject) {
      legacyJsonRequest('https://oauth2.googleapis.com/token', {
        ...
        // APPROVED DEVIATION -- the body is tested before it is dereferenced,
        // so a token response carrying no object rejects this chain instead of
        // terminating the process. The whole of the change is the `!body` test
        // on the line below.
        //
        // A token response that is empty, a bare `null` or a 3xx leaves `body`
        // undefined or null: POST is in UNFOLLOWED_METHODS, so a redirect
        // arrives as the response itself with nothing parsed. Baseline read
        // `access_token` straight off it, and because legacyJsonRequest
        // dispatches this callback with process.nextTick, the resulting
        // TypeError escaped as an uncaught exception rather than rejecting --
        // measured, one unauthenticated GET /auth/google/callback ended the
        // process on the first request. AAP 0.6.6 records that as preserved
        // under R-d.
        //
        // docs/preserved-quirks.md 10.7 supersedes R-d for a process death:
        // what the death destroys is not this branch's behaviour but every
        // other route's, and here the trigger is remote and unauthenticated
        // and the fault is the provider's. Nothing else moves with the guard.
        // The reject value, its message and the terminal catch's response are
        // untouched, so this edge now produces exactly what the seven other
        // provider profiles produce: `Google OAuth error: Failed to get access
        // token` logged, then the generic authentication failure, which for a
        // browser request flashes the message and redirects to /signup.
    ...
    .then(function(accessToken) {
      ...
      return new Promise(function(resolve, reject) {
        ...
        legacyJsonRequest('https://www.googleapis.com/oauth2/v2/userinfo', {
          ...
          // Same shape as the token guard, and the same approved deviation:
          // an empty or `null` profile body is refused here rather than
          // dereferenced. Baseline's read threw a TypeError out of this
          // next-tick callback and ended the process. The reject below carries
          // the value and message it always carried, so this edge lands on the
          // generic authentication failure alongside the other provider
          // profiles.
