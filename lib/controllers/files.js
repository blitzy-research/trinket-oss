...
module.exports = {
  uploadAvatar : async function(request, h) {
    ...
    // The wrapper resolves on both outcomes rather than rejecting, so the branch below decides
    // what the route answers instead of a rejection escaping to the centralized error map.
  ...
  upload : async function(request, h) {
    ...
    // Resolving on both outcomes is required here: lib/util/file.js#_fileToContainer calls back
    // as cb(unlinkErr, uploadResult), so `results` is populated even when `err` is set and the
    // body below reads it unconditionally. A rejecting wrapper would discard the result object
    // and turn the swallow below into a TypeError - a 500 where this route answers 200.
