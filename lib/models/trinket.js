...
function postRemove(doc) {
  ...
  if (doc.snapshot) {
    snapshot.removeSnapshot(doc.snapshot).catch(function(err) {
      ...
      //
      // The status is read from BOTH error shapes because the aws-sdk v2 -> @aws-sdk/client-s3 v3
      // replacement moved it: v2's AWSError carried a top-level `statusCode`, which is what this guard
      // was written against, while a v3 rejection carries it under `$metadata.httpStatusCode` and
      // leaves `statusCode` undefined. Reading only `statusCode` left the guard permanently open, so
      // the 404s the base commit swallowed - a snapshot object, or its whole bucket, already gone -
      // were logged instead. The v2 term is kept first so an error still carrying that shape is
      // honoured, and an error carrying neither status still logs, exactly as it did at base: the
      // usage-count rejection this same handler also receives is a plain Error.
      // See docs/PRESERVED-QUIRKS.md section 3.47.
