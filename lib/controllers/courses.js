...
module.exports = {
  ...
  create : async function(request, h) {
    ...
    // requirement. `courseCtrl.createCourseCore` enforces it for every caller
    // by throwing, which is what makes the shared creation path fail closed -
    // but a throw from the core lands in the catch below, whose non-duplicate
    // branch is a preserved non-response, so an unpermitted caller would get no
    // answer at all. The check is therefore repeated here, ahead of the core,
    // so this route answers through its own mapping: `fail.redirect` on
    // `POST /courses` is `/courses/new` (config/routes.js:133-135), so
    // `request.fail` flashes the message and sends the browser back to the
    // creation form, exactly as the duplicate-name case below does.
    if (request.payload && request.payload.courseType === 'private'
    && !(request.user && request.user.hasPermission('create-private-course'))) {
      return request.fail({
        message : "Your account doesn't include private courses. Please choose a different course type."
      });
    }
