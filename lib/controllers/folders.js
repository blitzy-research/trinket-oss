 ...
 * `create`'s duplicate-name branch does not arrive here either: it builds its own
 * 409 rather than take the 500 an unclassified Error maps to. Its
 * unknown-failure branch DOES arrive here, from inside a save callback, and takes
 * the Error mapping above.
 *
...
module.exports = {
  ...
  create : async function(request, h) {
    ...
    // failure branches below resolve too, each with the Boom its own comment
    // records, so every path the save callback itself takes now settles this
    // promise and answers the request. A promise that stays pending is still a
    // returned promise under T-1, but it answers nothing, and R-b does not permit
    // a route that never serves. The one path that still does not settle is the
    // DETACHED grant chain's rejection, which is baseline's own behaviour and is
    // recorded where it happens below.
    //
    // The executor takes no `reject`: nothing here rejects any more. The
    // duplicate-name branch was the only site that did, and its rejection was
    // exactly the uncontrolled 500 this file no longer answers with.
    return await new Promise(function(resolve) {
      folder.save(function(err, savedFolder) {
        if (err) {
          if (err.code === 11000) {
            // A duplicate name for the same owner - the `_owner_1_slug_1` unique
            // index this model declares - is answered HERE, as a conflict, instead
            // of being handed to the route catch-all as a server fault.
            //
            // This is a DEVIATION from baseline. The register entry that approves
            // it is docs/preserved-quirks.md §11.10 (Deviation 6); this comment
            // records what is delivered and cites that entry rather than
            // approving itself.
            //
            // Baseline cannot be preserved. At 2f8712a this branch calls
            // `request.catch({err, message})`, and `catch` is not a decoration
            // this application defines - lib/util/routeParser.js installs
            // request.success at :216 and request.fail at :280, and hapi's Request
            // carries no `catch` member - so the call throws a TypeError from
            // inside `folder.save(callback)`. Mongoose's
            // Model.$handleCallbackError re-emits that as an unlistened 'error'
            // event on the model, and with no 'uncaughtException' handler in
            // app.js, lib/ or config/, the process TERMINATES: one authenticated
            // duplicate name takes the whole server down, the client is answered
            // with nothing, and no route serves afterwards. AAP R-b requires the
            // application to run with no route excluded, so R-b controls over R-d
            // here - an absent response is not behaviour a client can depend on.
            // That is the precedence argument AAP 0.7 makes for the
            // never-settling stream at lib/controllers/files.js:98-100.
            //
            // A response therefore has to be invented, and it classifies the
            // failure as what it is - a client-caused conflict, not a server
            // fault. Delivered contract: HTTP 409, `application/json;
            // charset=utf-8`, body {"statusCode":409,"error":"Conflict",
            // "message":"You already have a folder with this name. Please choose
            // another."}, the process alive, and exactly one folder document
            // persisted. The message is not invented either: it is the string
            // baseline's own dead `request.catch` argument composes in this same
            // function, and the production client surfaces a `message` off this
            // response to the user
            // (public/js/library/components/folders/new-folder-directive.js:30-31,
            // :39-40). It interpolates no payload or key value, so the folder name
            // and the owner stay out of both the response and the log.
            //
            // RESOLVED, never thrown and never rejected: a Boom that is returned
            // is served with its own status, while a thrown or rejected one
            // reaches the handler catch-all (lib/util/routeParser.js:554-564),
            // which logs it and rewrites it to Boom.badImplementation - the
            // generic 500 that made a client conflict indistinguishable from a
            // server fault.
            return resolve(errors.conflict('You already have a folder with this name. Please choose another.'));
          }
      ...
      return folder.save()
        ...
        .catch(function(err) {
          if (err.code === 11000) {
            ...
            //
            // `create`'s duplicate branch could not be kept: the identical
            // expression there sat inside a save CALLBACK, where the same
            // TypeError took the process down instead of answering, and that is
            // the deviation recorded in docs/preserved-quirks.md §11.10. The two
            // duplicate paths therefore now answer differently - 409 from
            // `create`, this 500 from a rename - and that difference is the
            // deviation's exact boundary rather than an oversight.
