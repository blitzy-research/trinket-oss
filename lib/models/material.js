  ...
  if (this.type === 'assignment') {
    Trinket.findById(this.trinket.trinketId)
      ...
      .then(function(trinketCopy) {
        ...
        // rejection from Trinket.findById or trinket.copy never reaches cb. No `return` here, and the
        // two-argument .then keeps success and failure disjoint so a throw from cb is not fed back into cb.
        material.save().then(function(doc) {
          cb(null, doc);
        }, function(err) {
          cb(err);
        });
    ...
    parser.parse(materialData.content, user)
      .then(function(parsedContent) {
        ...
        // PRESERVED QUIRK - see docs/PRESERVED-QUIRKS.md. This chain is deliberately left unterminated: a
        // rejection from parser.parse never reaches cb. No `return` here, and the two-argument .then keeps
        // success and failure disjoint so a throw from cb is not fed back into cb.
        material.save().then(function(doc) {
          cb(null, doc);
        }, function(err) {
          cb(err);
