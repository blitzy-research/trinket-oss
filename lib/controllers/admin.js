var config        = require('config'),
    ...
    // SECURITY REMEDIATION (review findings SEC-13 / M6 / SV-03, CWE-200 + CWE-522). The single
    // centralized credential scrub. Both surfaces below JSON-clone the User document in the HANDLER,
    // which flattens it before any responder runs, so the shared fix in lib/models/model.js#serialize
    // cannot reach them and each applies the scrub itself. See lib/util/credentials.js and
...
module.exports = {
  ...
  ohnoes : async function(request, h) {
    ...
    // would delay the 200 that has already been computed above.
    //
    // The terminal catch does NOT change behavior (review finding F7). The alert's outcome was
    // never reported to the client and still is not; nothing about the arguments, the ordering or
    // the response moves. What it prevents is a rejection from a CONFIGURED transport escaping
    // with no handler, which Node 22's default `--unhandled-rejections=throw` turns into a
    // process-fatal fault - one unreachable SMTP host would take the whole server down from an
    // admin route that had already answered 200. The catch is empty, and logs nothing, because the
    // base commit had no failure path here either: a failed alert was, and remains, invisible.
  ...
  uploadUsers : async function(request, h) {
    ...
    try {
      ...
      //
      // R-6, and measured rather than assumed - see lib/http/responseContract.js#rejectOrAbandon for
      ...
      // deferred unsettled: this branch answered NOTHING. rejectOrAbandon invokes the same
  ...
  updateUser : async function(request, h) {
    ...
    try {
      ...
      // rejection and this branch answered NOTHING. See lib/http/responseContract.js#rejectOrAbandon.
      return NoResponse.rejectOrAbandon(h, err);
    }
    ...
    if (request.payload.roles) {
      ...
      try {
        ...
        // lib/http/responseContract.js and docs/PRESERVED-QUIRKS.md.
        return NoResponse.rejectOrAbandon(h, err);
      }
    ...
    // no body, no log line.
    //
    // That outcome IS reproducible, so it is reproduced rather than converged: `h.abandon`
    // is hapi's own no-response outcome, so this one request is left unanswered with its
    // socket open while the server and every other route carry on normally. Letting the
    // handler fall through instead would resolve `undefined` and make hapi answer 500 - a
    // status no baseline request on this branch ever received, and a TR2 parity failure. An
    // else branch, a default response or a validation guard would be latent-bug repair,
    // which R-1 excludes. See lib/http/responseContract.js for the measured semantics.
    return h.abandon;
  },
  grantRole : async function(request, h) {
    ...
    return user.grant(request.payload.role, "site")
      ...
      .then(function(user) {
        ...
        // docs/PRESERVED-QUIRKS.md section 4.14. The `JSON.parse(JSON.stringify(user))` round-trip is
        // the base commit's payload SHAPE and is kept as the payload shape: it clones the whole User
        // document rather than projecting `publicSpec`, which is why this response disclosed ANOTHER
        // user's bcrypt hash to the admin caller.
        //
        // An earlier revision deleted the top-level `password` key alone, which was not enough.
        // `User.schema.profiles` is an untyped Mixed object (lib/models/user.js:L18) and
        // lib/controllers/auth.js:L232/L253 persist a LIVE Google OAuth bearer credential into it at
        // `profiles.google.token`, so the clone still carried another user's provider token here. The
        // scrub is therefore the centralized recursive one, which removes every credential-named key at
        // every depth and leaves all other keys byte-identical. Nothing in the repository reads this
        // response - the admin role editor at lib/views/admin/index.html posts to
        // /api/admin/user/{id} and reads only `result.success` - so the removal is observably neutral
      ...
      .catch(function(err) {
        // R-6 - see lib/http/responseContract.js#rejectOrAbandon. At the base commit this chain was
...
async function userSearch(q) {
  ...
  // SECURITY REMEDIATION (review findings SEC-13 / M6 / SV-03) - see
  // docs/PRESERVED-QUIRKS.md section 4.14. This round-trip is the base commit's shape and stays the
  // shape, but it flattens the User document to a plain object BEFORE the success responder runs, so
  // `ObjectUtils.serialize`'s prototype-safe `typeof(json.serialize)` test finds nothing to project and
  // the whole document reaches the client. That is how the searched user's bcrypt hash arrived in the
  // rendered admin page, which dumps this object wholesale through `{{ data | json("pretty") | safe }}`
  // in lib/views/admin/includes/users.html - a frozen template, so the credentials are removed here at
  // the source instead. The template's named reads are `_id`, `avatar`, `coursesOwned`, `email`, `name`,
  // `roles`, `trinketsOwned` and `username`; no credential is among them, and `roleSearch` below already
  // answers clean because it hands the responder real documents.
  //
  // The scrub is the centralized recursive one rather than a top-level `delete data.password`, because
  // `profiles.google.token` - a live OAuth bearer credential written by lib/controllers/auth.js - sits
  // one level down inside the untyped Mixed `profiles` object and survived the key-by-key delete. Every
  // non-credential key of this object is untouched.
