...
env.addFilter('cachePrefix', function(src, key) {
  // PRESERVED QUIRK - see docs/PRESERVED-QUIRKS.md. StringUtils.addPrefix reads
  // config.app.prefixes[key] and falls back to a hardcoded '/cache-prefix-' + Date.now() when it is
  // unset. All eight entries in config/default.yaml (components, js, css, img, fonts, partials,
  // skulpt, models) are NULL, so the hardcoded branch ALWAYS fires and app.cachePrefix is never
  // read. TR5-frozen client-visible asset URLs: do not "fix" it here or in stringUtils.js.
...
env.addFilter('userAvatar', function(str) {
  ...
  // PRESERVED QUIRK - see docs/PRESERVED-QUIRKS.md. config/default.yaml ships
  // aws.buckets.useravatars.host as 'https://your-avatars-bucket.example.com', so this exclusion
  // never passes and the '/img/' + str fallback below is the DEFAULT baseline branch. All five
  // branches, both regexes and both '/img/' literals are frozen client-visible asset URLs (TR5).
...
function escapeJSON(data) {
  ...
  if (data instanceof Array) {
    ...
    // PRESERVED QUIRK - see docs/PRESERVED-QUIRKS.md. The two lines above are the author's own
    // record of that choice, and it stands: lodash stays the escaper even though underscore is
    // already required here as _. The 4.17.23 -> 4.18.1 bump was verified byte-identical across
    // every BMP code point, so escaped markup is unchanged on both sides of the wire.
