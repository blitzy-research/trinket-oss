var mongoose  = require('mongoose')
  ...
  // `isEmail` comes from lib/util/email-compat.js rather than straight from
  // `validator`, because the move from validator 5.7.0 to 13.15.35 changed the
  // verdict for gmail local parts containing dots, local parts containing
  // U+00A0, and addresses of 255 characters. Those verdicts are persisted as
  // the `status` field below and decide whether an invitation is ever mailed,
  // so the compat module reproduces 5.7.0's answers. See that file's header.
  , validator = require('../util/email-compat')
