...
module.exports = {
  ...
  login: async function(request, h) {
    if (request.auth.isAuthenticated) {
      ...
      // hands it to h.redirect() verbatim - so only a same-origin root-relative
      // destination is persisted. Redirect.internalDestination returns the value
      // UNCHANGED for every in-application path, so a legitimate
      // '/login?next=/courses/x' still round-trips byte-for-byte; an absolute,
      // scheme-relative, backslash or control-character destination is simply not
      // stored, which leaves the session exactly as it is for a visitor who
      // supplied no `next` at all.
  ...
  features : async function(request, h) {
    ...
    // bounds). The asymmetry with lib/controllers/trinket.js:L1029, which reads the same pre
