var mongoose    = require('mongoose'),
    ...
    // centralized credential scrub, applied in serialize()'s nested-clone branch below - the shared
    // mechanism by which a populated sub-document reaches the wire whole. See lib/util/credentials.js
      ...
      for (var key in publicSpec) {
        if (Array.isArray(this[key])) {
          ...
          if (this[key].hasOwnProperty('serialize') && typeof(this[key].serialize) === 'function') {
            ...
            //
            // SECURITY REMEDIATION (review findings SEC-13 / M6 / SV-03, CWE-200 + CWE-522), recorded in
            // docs/PRESERVED-QUIRKS.md section 4.14. THIS BRANCH IS THE ROOT CAUSE, and it is reached
            // far more often than it looks: the test above is `hasOwnProperty('serialize')`, but
            // mongoose installs `serialize` on the document PROTOTYPE, so it is FALSE for every
            // populated sub-document and every one of them lands here and is cloned WHOLE.
            // `Course.publicSpec` whitelists `_owner` and `setOwner` assigns the populated User
            // document to it, which is how POST /api/courses - and POST /courses through the
            // server.inject in lib/controllers/courses.js#create - shipped the owner's bcrypt hash and,
            // for a Google-linked owner, the live OAuth bearer credential at `profiles.google.token`
            // that lib/controllers/auth.js writes into the untyped Mixed `profiles` object.
            //
            // The clone is the base commit's payload SHAPE and stays the shape; only values no client
            // may legitimately read are removed, so every other key is byte-identical. Fixing it here
            // rather than per route closes the class for any future model with a populated
            // sub-document. The scrub is a deny-list of exact credential names, censused against all 24
            // models before being applied generically: the only matching schema field is
            // `User.password`, and `CourseInvitation.token` is a top-level scalar that reaches the
            // `else` branch below rather than this one. `hasOwnProperty` is deliberately NOT loosened
            // to a `typeof` test - that would project sub-documents through `publicSpec` and drop
            // `roles`, `verified`, `coursesOwned` and `trinketsOwned`, which the admin page and the
            // course response genuinely read.
