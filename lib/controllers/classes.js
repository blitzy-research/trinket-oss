...
module.exports = {
  ...
  acceptInvitation : async function(request, h) {
    ...
    // invitation names before anyone is added to the course. `lib/models/
    // courseInvitation.js` also mints the token from the CSPRNG now, so a
    // resolved token is no longer a value an outsider can derive or search for.
