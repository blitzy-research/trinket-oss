...
/**
 * Bridge the two handler idioms this file has to serve at the same time.
 *
 * A lifecycle method written for the legacy surface CALLS its second argument --
 * reply(data) -- and signals its response out of band. A lifecycle method written
 * against the hapi contract names that argument `h` and builds its response
 * through the toolkit: h.response(...), h.redirect(...), h.view(...). Both are
 * handed the same single argument, so that argument has to be callable and also
 * expose the toolkit.
 *
 * The returned value is the reply function itself: calling it, and the chainable
 * builder it returns, behave exactly as before. Only property reads the reply
 * function does not define fall through to the real toolkit, bound to it.
 */
function toolkitAwareReply(replyFn, h) {
  return new Proxy(replyFn, {
    ...
    has : function(target, prop) {
      return Reflect.has(target, prop) || (prop in h);
    }
  });
}
      ...
      if (typeof preHandler.method === 'function') {
        ...
        var wrappedMethod = async function(request, h) {
          return new Promise(function(resolve, reject) {
            ...
            try {
              var result = originalMethod(request, toolkitAwareReply(fakeReply, h));
    ...
    // Handle string format: 'isAdmin(user)'
      ...
      var wrappedFunc = async function(request, h) {
        return new Promise(function(resolve, reject) {
          ...
          try {
            var result = originalFunc(request, toolkitAwareReply(fakeReply, h));
            if (result && typeof result.then === 'function') {
              result.then(function(val) {
                resolve(val === undefined ? null : val);
              }).catch(reject);
  ...
  routeConfigs.forEach(function(route, index) {
    ...
    route.handler = async function(request, h) {
      ...
      try {
        ...
        // Handlers converted to the hapi lifecycle contract build their response
        // through the toolkit off this same argument, so expose it here.
        reply = toolkitAwareReply(reply, h);
