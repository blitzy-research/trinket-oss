  ...
  createCourse : async function(request, h) {
    ...
    return course.addUser(request.user, ["course-owner"])
      .then(function() {
        ...
        // bcrypt hash in this body and, through the server.inject consumer below, in POST /courses
        // too. The owner is therefore replaced with the very clone that branch would have produced,
        // minus the one credential key: every other key stays byte-identical, so the only
        // observable difference is the absence of a value no client can legitimately read. The
        // clone is fresh, so `request.user` - which `setOwner` aliased here - is not mutated.
        if (course._owner && typeof course._owner.toJSON === 'function') {
          var owner = JSON.parse(JSON.stringify(course._owner));
