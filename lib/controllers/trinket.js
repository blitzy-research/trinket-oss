var mailer        = require('../util/mailer'),
    ...
    // The email-share token's key material, derived in exactly one place for
    // both the three signing sites below and helpers.verifyEmailToken, which is
    // what keeps the two sides of that token from disagreeing. Acyclic: this
    // module requires ./store, ./features, config, @hapi/boom, underscore,
    // jsonwebtoken and crypto, and no controller.
...
// COMPRESSED input only, so decompression needs its own limits. DEVIATION 3, NOT
// PRESERVATION: preserved-quirks 11.0 registers it, 11.6 argues it, 10.7 measures it.
...
// Characters that must never reach a template path built from a request parameter:
// NUL, the C0 controls and DEL. Only NUL is fatal - it is the one value `fs.stat`
// refuses with a THROW rather than an errno - but no control character can appear in
// a legitimate template name, and every one of them resolves to the same missing
// template as any other unknown value, so refusing the whole class costs no reachable
// behaviour. See `beta` below, which is the only route in this controller whose view
// name carries an unvalidated path parameter.
var UNSAFE_VIEW_PARAM = /[\x00-\x1f\x7f]/;
...
//
// NONE of the containment is delegated to archiver: the declared range has moved
// once already in this migration (2.1.1 -> 7.0.1, to clear the DEP0005 its
// compress-commons 1.2.2 emitted), so a control resting on the library's own
...
module.exports = {
  ...
  // `GET /embed/beta/{type}`, whose declaration carries `html: 'embed/beta/{type}.html'`
  // (config/routes.js). `type` is projected into the success payload below and
  // lib/util/routeParser.js interpolates the template out of that payload
  // (`StringUtils.interpolate(template, json)`), so this handler is where an
  // attacker-controlled path parameter becomes a FILESYSTEM TEMPLATE PATH. It is the
  // only route in this controller that does that: the six routes declaring
  // `embed/{lang}.html` are gated by `trinketTypeEnabled`/`validLang` before their
  // handlers run - MEASURED on this tree, `/embed/foo%00bar`,
  // `/embed/foo%00bar/{id}`, `/tools/v1/jekyll/embed/foo%00bar`,
  // `/assignment-embed-viewonly/foo%00bar/{id}` and `/embed/python%00/{id}` all
  // answer 404 and `/assignment-embed/foo%00bar/{id}` answers 302, every one of them
  // with the process still serving.
  //
  // THE GUARD, and why it is here rather than in the declaration or the parser.
  // A percent-encoded NUL survives hapi's path decoding, reaches `fs.stat` through
  // @hapi/vision's `Manager._path` (node_modules/@hapi/vision/lib/manager.js:333) and
  // Node throws `TypeError [ERR_INVALID_ARG_VALUE]`. Vision re-throws it through
  // `Bounce.rethrow(err, 'system')` because a TypeError is a system error, and it
  // escapes the request lifecycle AFTER this handler has already returned, so no
  // `try` in this file and no route catch-all can see it: the client is answered with
  // nothing and the PROCESS EXITS, taking all 233 routes and every concurrent request
  // with it. MEASURED identically on this tree and at base commit 2f8712a -
  // `curl --path-as-is /embed/beta/foo%00bar` -> curl exit 52, empty reply, and a
  // following `GET /` cannot connect. Unauthenticated and repeatable.
  //
  // Preserving that outcome is impossible under R-b ("the application must genuinely
  // run ... with no route or module excluded"), and AAP 0.7 already settled this
  // precedence for the sibling case at lib/controllers/files.js:98-100: an absent
  // process is the absence of a response rather than behaviour a client can depend
  // on. So the value is refused HERE, before `request.success` builds the view, which
  // is the only place inside this controller that can act before the escape happens -
  // config/routes.js:446-459 records the same hazard and states that the remedies
  // available at the declaration would move either the 233-route manifest (AAP 0.9.1)
  // or the 102-target validation inventory (0.6.2), neither of which may move.
  //
  // THE RESPONSE IS THE ROUTE'S OWN. `embed/beta/` names a template directory that
  // does not exist anywhere in this repository, so EVERY `{type}` resolves to a
  // missing template and answers 500 - vision's own `Boom.badImplementation('View
  // file not found')` rendered as lib/views/50x.html by app.js's onPreResponse.
  // MEASURED on both trees: `/embed/beta/python` and `/embed/beta/harmless-unknown-slug`
  // both answer 500 with the same 1,600-byte page. Returning that same Boom for a
  // refused parameter therefore reproduces the status, content type and body this
  // route already produces for every input that gets a response at all, so R-e's
  // "same status codes" holds and nothing but the process death changes. Returned
  // rather than thrown, as everywhere else in this file: a thrown Boom is rewritten
  // by the route catch-all, which would keep the status but replace the message.
  beta : async function(request, h) {
    if (UNSAFE_VIEW_PARAM.test(String(request.params.type))) {
      return errors.badImplementation('View file not found');
    }
  ...
  list : async function(request, h) {
    ...
    try {
      ...
      // The route's `reply` projection turns this into `{data:[...]}` and adds
      // `flash` and `context`. No count, page or cursor metadata accompanies
      // it, so a caller cannot determine how many pages exist -- recorded as
      // characteristic (4) on `listCore` below, along with the rest of this
      // listing's measured pagination behaviour.
  ...
  // Shared by the name, description and published routes.
  //
  // PRESERVED BASELINE BEHAVIOUR -- an empty trinket name is accepted. The name route
  // declares `name : Joi.string().allow('').max(50)` (config/api_routes.js), so `""`
  // passes validation, is set here and is saved. MEASURED: `PUT
  // /api/trinkets/{id}/name` with `{"name":""}` -> 200 `{"success":true}` and the
  // stored `name` becomes `""`. The sibling folder field carries a genuinely different
  // declared contract at baseline -- `Joi.string().min(1).max(140).required()` -- so
  // `POST /api/folders` with `""` refuses the value (measured: 200 carrying
  // `flash.validation.name` = "\"name\" is not allowed to be empty", no folder
  // created, since the hand-rolled validation block routes failures through
  // `request.fail` rather than a 400).
  //
  // NOT FIXED HERE: requiring a non-empty name means editing a route declaration AAP
  // 0.2.1 leaves untouched, and it would change this route's accept/reject outcome --
  // prohibited by 0.7 R-d and pinned by the joi matrix (0.6.2) and the captured
  // corpus (0.9.3).
  ...
  create : async function(request, h) {
    ...
    // PRESERVED BASELINE BEHAVIOUR -- `_owner` is set ONLY when `?library=true` is
    // present, while `_creator` is set for any signed-in caller. A trinket created
    // without the flag therefore persists `_owner: null`, and `canEditTrinket` above
    // -- which authorizes on `_owner` -- then denies every mutating route to the
    // account that created it. MEASURED as the creator on such a document: PUT
    // /name, PUT /code, PUT /published and DELETE all 403, GET 200; nothing in the
    // application can reclaim or delete it. This is the shape anonymous sandbox
    // trinkets rely on, so the null owner is load-bearing rather than incidental.
    //
    // NOT FIXED HERE: assigning an owner unconditionally, or widening the edit check
    // to `_creator`, changes who may mutate a document -- a behaviour change AAP 0.7
    // R-d prohibits, and the anonymous `POST /api/trinkets` case is captured in the
    // baseline corpus (0.9.3).
    if (request.user) {
    ...
    return trinket.save()
      .then(function(doc) {
        // The share-token key comes from helpers.emailTokenSecret, which is the
        // one place either side of this token derives it. A null return means no
        // key may exist on this process - production with app.mail.secret unset
        // - and nothing is minted: no signature, no session entry, so the share
        // modal's hidden field renders empty and the share route answers its own
        // 400 rather than accepting a token signed with a derivable key.
        emailSecret = helpers.emailTokenSecret(doc.shortCode);
  ...
  createFork : async function(request, h) {
    ...
    return Trinket.findByIdAndUpdateMetrics(parent.id, 'forks', meta)
      ...
      .then(function(savedTrinket) {
        // Same contract as `create` above: the key comes from
        // helpers.emailTokenSecret, and a null return mints nothing.
        emailSecret = helpers.emailTokenSecret(savedTrinket.shortCode);
  ...
  // PRESERVED BASELINE BEHAVIOUR -- an unauthenticated request can create a trinket
  // here, and that is deliberately unchanged. Two properties of the route combine:
  //
  //   * `PUT /api/trinkets/{trinketId}/metrics` declares neither `auth` nor `pre`
  //     (config/api_routes.js), so the server default `mode: 'try'` (app.js) applies
  //     and there is no `trinket(params.trinketId)` pre-handler to 404 an unknown id.
  //   * `Trinket.findByIdAndUpdateMetrics` passes `{new: true, upsert: true}` to
  //     `findByIdAndUpdate` (lib/models/trinket.js), which does not run the pre-save
  //     `hashify` hook.
  //
  // MEASURED, cookie-less, at an id that does not exist: 200 with
  // `{"data":{"metrics":{"runs":1}}}` and one inserted `snippets` document carrying
  // `code:""`, `lang:"python"`, `_owner:null`, `_creator:null` and -- because hashify
  // is bypassed -- `hash:null`, `shortCode:null`. It is self-limiting rather than
  // unbounded: `shortCode_1` is `unique` and NOT sparse, so a second such insert
  // fails E11000 and the route answers an uncontrolled 500 (measured).
  //
  // NOT FIXED HERE, BY THE PLAN'S OWN RULES. The remedy is a pre-handler plus dropping
  // `upsert`, and both lie outside what this migration may change: AAP 0.2.1 scopes
  // config/api_routes.js to the single inline pre-handler on POST /api/users/login and
  // leaves "its 116 declarations untouched", lib/models/** is out of scope entirely, and
  // AAP 0.7 R-d prohibits behaviour improvements while 0.9.1 requires the 233-route
  // manifest -- effective auth and pre-handlers included -- to stay identical to
  // baseline. Reclassifying this route needs its own approved change.
  ...
  embed : async function(request, h) {
    ...
    return promise
      .then(function(result) {
        ...
        if (trinket.shortCode) {
          // Same contract as the two save paths: the key comes from
          // helpers.emailTokenSecret, and a null return mints nothing - so
          // `emailToken` stays undefined and the shareModals hidden field at
          // lib/views/includes/shareModals.html:135 renders empty, exactly as
          // it does for a trinket with no shortCode. The `library` test that
          // decides whether the token is ALSO written to the session is
          // unchanged and stays nested where it was.
          emailSecret = helpers.emailTokenSecret(trinket.shortCode);
  ...
  logError : async function(request, h) {
    ...
    // persisted -- or has failed -- before the client is told anything. Baseline
    // does the same by ignoring the `err` argument of the save callback.
    //
    // PRESERVED BASELINE BEHAVIOUR -- the swallow hides a real data loss whenever the
    // route schema is looser than the model. `POST /api/trinkets/codeerror` validates
    // `state` as `Joi.string().required()` (config/api_routes.js) while the model
    // constrains it to `enum: ["encountered","repeated","resolved"]`
    // (lib/models/errorEvent.js), so an out-of-enum `state` passes validation, fails
    // the save with a Mongoose ValidationError, is swallowed here, and the client is
    // told 200. MEASURED: `state:"encountered"` -> 200 and `errorevents` 0 -> 1;
    // `state:"s1"` -> 200 with the count frozen at 1 and nothing persisted.
    //
    // NOT FIXED HERE. Mirroring the enum in the route schema would edit a declaration
    // AAP 0.2.1 leaves untouched and would flip a recorded outcome in the joi parity
    // matrix (0.6.2, all 102 targets); surfacing the save error would change this
    // route's status, which AAP 0.7 R-d prohibits and the captured baseline corpus
    // (0.9.3) pins at 200.
  ...
  // PRESERVED BASELINE BEHAVIOUR -- this route does not require the parent trinket to
  // exist, while the route that reclaims a draft does. `POST
  // /api/trinkets/{trinketId}/draft` declares `auth: 'session'` and `payload.maxBytes`
  // but NO pre-handler, whereas `DELETE /api/drafts/{trinketId}` declares
  // `pre: ['trinket(params.trinketId)']` (config/api_routes.js). `Draft.findOneAndUpdate`
  // then upserts against whatever `trinketId` was in the path.
  //
  // MEASURED, as an ordinary signed-in user, at an id with no `snippets` document:
  // POST -> 200 `{"success":true,...}` and a draft is stored; the paired DELETE -> 404,
  // so nothing in the application can reclaim it. A malformed id -> 200
  // `{"flash":{},"context":null}`, because the cast failure is swallowed by the
  // `.catch` at the end of this handler, which answers `request.success()` either way.
  // The route's 10 MB `payload.maxBytes` bounds each such record, not their number.
  // Both properties are byte-identical at base 2f8712a, route declarations included.
  //
  // NOT FIXED HERE: adding the parent pre-handler edits a declaration AAP 0.2.1 leaves
  // untouched and moves the 0.9.1 manifest off baseline (161 routes carry pre-handlers);
  // surfacing the cast failure changes this route's status, which 0.7 R-d prohibits and
  // the captured baseline corpus (0.9.3) pins at 200.
  draft : async function(request, h) {
    ...
    // PRESERVED BASELINE BEHAVIOUR -- `settings` is a whole-subdocument replace, not a
    // merge. The payload object is assigned to `update.settings` as a unit and
    // `Draft.findOneAndUpdate` (lib/models/draft.js) hands it to Mongoose's implicit
    // `$set`, so keys absent from THIS request revert to their schema defaults
    // (`autofocusEnabled: true`, `testsEnabled: false`) even though the route accepts
    // `Joi.object().unknown(true)`. MEASURED: writing
    // `{autofocusEnabled:false,testsEnabled:true}` persists both; a following
    // `{autofocusEnabled:true}` persists `{autofocusEnabled:true}` alone and the
    // stored `testsEnabled:true` is lost.
    //
    // NOT FIXED HERE: a dot-notation merge would change what a partial write persists.
    // AAP 0.7 R-d prohibits the improvement, lib/models/** is out of scope, and this
    // route is captured in the baseline corpus (0.9.3).
    ...
    if (request.payload.zipCode) {
      ...
      // At baseline nothing handled that rejection and nothing listened for
      // 'unhandledRejection', so the process exited after the client had been
      // answered; the terminal `.catch` below is what now prevents that exit.
  ...
  discardDraft : async function(request, h) {
    // PRESERVED BASELINE BEHAVIOUR -- the reported outcome does not depend on whether
    // anything was deleted. The query is correctly scoped to the calling user, so one
    // account can never discard another's draft; but `deleteMany`'s result is not
    // consulted, so a cross-owner request answers exactly like a successful one.
    // MEASURED: user B issuing `DELETE /api/drafts/{trinketId}` against user A's draft
    // -> 200 `{"success":true}` with the `drafts` count unchanged.
    //
    // NOT FIXED HERE: reporting the no-op (a 404, or `success:false`) changes this
    // route's response -- prohibited by AAP 0.7 R-d and pinned by the captured
    // baseline corpus (0.9.3). The scoping that makes it safe is already correct and
  ...
  autosave : async function(request, h) {
    ...
    if (request.user.id.toString() === request.pre.trinket._creator.toString()) {
      ...
      if (request.payload.zipCode) {
        ...
        // JSON.parse on it, which throws. At baseline nothing handled that
        // rejection and nothing listened for 'unhandledRejection', so the process
        // exited after the 500 was sent; the terminal `.catch` below prevents that.
        return await new Promise(function(resolve) {
          zip.loadAsync(request.payload.zipCode, { base64: true })
            .then(function(content) {
  ...
  downloadPostedZip : async function(request, h) {
    ...
    // CLAIMS THE REJECTION IN THE SAME SYNCHRONOUS TURN THE PROMISE IS CREATED, and
    // does nothing else. The chain that eventually awaits `outputPromise` is not
    // attached until `Promise.all(assetPromises)` settles, while `archive.append`
    // below emits its 'error' SYNCHRONOUSLY for a source that is neither a Buffer nor
    // a stream - which is any `files` value that is not a string, this route being
    // unauthenticated. With a real asset download in flight the microtask queue
    // therefore drains with this promise rejected and unclaimed, and Node 22's default
    // unhandled-rejection behaviour throws: measured as `http=000 rc=52` and process
    // exit 1 on `ArchiverError ... INPUTSTEAMBUFFERREQUIRED`, taking every concurrent
    // in-flight request with it and orphaning the 0700 directory created above. The
    // base commit dies identically, so it is pre-existing rather than introduced by
    // the conversion, and app.js listens for neither 'unhandledRejection' nor
    // 'uncaughtException', which is why the default applies.
    //
    // AAP 0.7's decision on lib/controllers/files.js:98-100 governs the precedence:
    // R-b's "every route serves" outranks R-d for the ABSENCE of a response, because
    // an unanswered request is not behaviour a client can depend on and because the
    // intended response is present in the same function rather than inferred - the
    // `.catch` that terminates this chain already maps the failure to
    // `errors.badImplementation`, and this same route already answers 500 for this
    // same malformed `files` value whenever no asset makes the assembly asynchronous.
    // So no error mapping changes here (R-e): the rejected value still reaches that
    // `.catch`, which still removes the private directory and still builds the same
    // response. The handler is empty ON PURPOSE - claiming the rejection must not
    // answer the request, and attaching it in this turn rather than later is what
    // keeps it off the unhandled path without emitting PromiseRejectionHandledWarning.
    outputPromise.catch(function() {});

    // DIRECTORY OWNERSHIP for the synchronous window. Everything between the
    // `mkdtempSync` above and the promise chain below runs synchronously, and three
    // things in it throw: `assets.forEach` on a payload whose `assets` parsed to a
    // non-array (`assets=null` parses to null, which JSON.parse does not reject),
    // `parseLegacy` on a malformed asset URL (ERR_INVALID_URL), and `archive.append`
    // on a malformed source. Each of those left the 0700 directory behind holding the
    // partial archive built before the throw, because the cleanup was reachable only
    // from the read stream's events and from the chain's own `.catch`, neither of
    // which exists yet at this point. Measured before this guard: one leftover
    // directory per failing request, and seven from a concurrent set.
    //
    // The throw is RE-THROWN unchanged, so it still leaves this handler exactly as it
    // did and still reaches the route catch-all that answers 500 - the status and
    // payload of every one of these paths is untouched (R-e). Only the leftover goes.
    // `removeZipDir` is idempotent (`force: true`) and fire-and-forget, so calling it
    // here costs the response nothing and cannot double-fail when a later terminal
    // path calls it too. AAP 0.6.7 requires cleanup on success AND failure, which is
    // what makes this a defect rather than a preserved quirk; moving `mkdtempSync`
    // below the loops was rejected instead, because it would reorder the write-stream
    // setup of a corpus-bound response against its `archive.append` calls.
    try {
      ...
      //
      // A malformed asset URL throws ERR_INVALID_URL synchronously out of this loop,
      // before the promise chain below exists. That throw must keep reaching the route
      // catch-all, because the 500 it produces there is this route's measured baseline
      // response, so the guard enclosing this region re-throws it unchanged rather
      // than answering locally. What the guard does add is the cleanup: this path used
      // to leave the private directory behind holding whatever had been appended
      // before the throw (measured: 56 bytes for a one-code-file request), because
      // neither the chain's `.catch` nor the read stream's events exist yet.
      //
      // Creating the directory after this loop would also close the leftover, but it
      // moves the stream setup of a corpus-bound response relative to its
      // `archive.append` calls - a larger behavioural risk than the leftover it
      // removes. Cleaning up in an `onPreResponse` extension would need a
      // request-scoped registry this controller does not have and app.js does not
      // consult. Re-throwing from an enclosing `try` needs neither.
...
//
// ---------------------------------------------------------------------------
// MEASURED PAGINATION CHARACTERISTICS, AND WHY THEY STAND
// ---------------------------------------------------------------------------
// Five characteristics of this listing were measured and are recorded here
// because each one is easy to mistake for an oversight and "correct" by
// accident. Every one is present identically on the pre-migration tree at
// 2f8712a and on this one, so none of them is a product of the hapi 21 / async
// conversion; and every one of them is a deliberately PRESERVED behaviour, not
// a defect awaiting repair in this delivery. AAP R-d prohibits behaviour
// improvements and directs that a quirk be preserved and documented; AAP R-a
// bounds the diff to the runtime bump, the hapi API migration, the async
// conversion and blocking-only dependency swaps. A change to any of the five
// below is a fifth category of change and needs its own authorization.
//
// (1) Page cost is governed by the owner's total collection size, not by the
//     requested page. Measured on this tree at 10,004 owned trinkets:
//     `?limit=1`, returning ONE document, took a 63.8 ms median against 5.7 ms
//     at 4 documents; `?offset=999999&limit=10`, returning ZERO documents, took
//     79.7 ms. `explain` shows IXSCAN on the single-field `_owner` index (the
//     only one available -- see lib/models/trinket.js) examining every owned
//     document, and the `$sort` below is a blocking in-memory stage for every
//     one of the five permitted sort keys.
//     The reason no index removes the sort cost is structural and sits in this
//     function: the sort keys are COMPUTED fields that the three `$project`
//     stages below invent -- `lastViewed` from `$ifNull` on lastView.viewedOn,
//     `totalViews` from `$add` of the three metric counters, `lowerName` from
//     `$toLower` of a `$cond`. None of them exists in a stored document, so
//     they cannot be indexed. This was tested rather than reasoned about:
//     creating a `{_owner, <sort key>}` compound index for each of the five
//     keys left `docsExamined` unchanged, left the `$sort` blocking, and was
//     not chosen by the planner at all. What DOES reduce the examination ratio
//     is an index over the MATCH predicate below rather than the sort --
//     `{_owner, folder, deletedAt}` took `docsExamined` from 10,004 to 22 and
//     the median from 45.9 ms to 5.2 ms when most of the owner's trinkets were
//     filed, and made it WORSE (63.8 ms to 95.4 ms) when none of them were,
//     because there is then no selectivity to exploit. That index would live in
//     lib/models/trinket.js, which this migration leaves unchanged (AAP §0.3.1),
//     so it belongs to whoever authorizes the follow-up -- with the caveat that
//     its benefit is conditional on the shape of the owner's collection.
//
// (2) `limit` has no ceiling. `?limit=1000000000`, `?limit=100000` and
//     `?limit=9007199254740993` each returned the entire 10,004-document
//     collection in one 4,601,876-byte response (measured); eight concurrent
//     such requests moved RSS from 144,720 kB to 351,160 kB and stretched a
//     concurrent one-document request from a 68.7 ms median to 90-219 ms. The
//     ceiling is absent in two places at once and both would have to move
//     together: the `$limit` stage at the end of this function, and
//     `limit : Joi.string().optional()` on GET /api/trinkets
//     (config/api_routes.js), whose Joi schemas AAP §0.2.1 holds untouched.
//     Note that the shape of the fix already exists elsewhere in the codebase
//     and was deliberately not applied here: the sibling listings
//     GET /api/trinkets/popular and GET /api/trinkets/active declare
//     `Joi.number().optional().max(100).min(0)`, GET /api/exports declares
//     `max(50)`, and lib/models/plugins/paginate.js enforces a `maxLimit` of
//     100 by throwing -- but this function does not go through that plugin, and
//     `?limit=101` returning 101 rows (measured) is the direct proof of that:
//     the plugin's ceiling is real and simply not on this path.
//
// (3) FOUR query keys answer 500 rather than rejecting the input, each by its
//     own mechanism, and every one of them measured as `{"statusCode":500,...}`:
//       - an unpermitted `sort` -- `Joi.string().optional()` accepts every
//         string, and the `sortMap` lookup below then throws on `undefined[0]`;
//       - `?sort=__proto__`, `?sort=constructor` and `?sort=toString`, which
//         resolve to INHERITED members of the map instead of undefined and pass
//         an undefined key into mongod's `$sort`;
//       - a negative `limit` or `offset`, which reach mongod and are refused
//         there (a limit of 309 digits becomes Infinity and is refused too);
//       - a malformed `folder`, where `new ObjectId(options.folder)` in the
//         match block above throws a BSONTypeError synchronously. A
//         syntactically valid but absent 24-hex id answers 200 with no rows.
//     The malformed-`folder` case is worth noting because it is not merely
//     preserved, it is PINNED: the accepting case that test/parity/joi-baseline.json
//     records for this route sends `folder:"aaa"` and records status 500, so
//     the §0.6.2 matrix would report a difference if it ever answered anything
//     else.
//     A 400 is not reachable from here even with a tightened schema: a
//     validation rejection goes through routeParser's `request.fail`, which
//     answers a redirect for an HTML request and 200-with-flash for a JSON one,
//     and AAP §0.3.1 lists that funnel as explicitly untouched.
//
// (4) There is no pagination envelope and the limit coercion is lossy. The
//     response carries `data`, `flash` and `context` and no total, next or
//     page count, so a caller cannot know how many pages exist; and
//     `parseInt(...) || 20` yields 20 items for `limit=0`, 16 for `0x10`, 1 for
//     `1e3` and for `1.5`, 5 for `5junk`, 20 for `abc`, `NaN`, `Infinity`,
//     `.5`, `-0.5` and whitespace, and the whole collection for
//     `9007199254740993` (all measured) -- so it reads a leading numeric prefix
//     where there is one and silently falls back to 20 where there is not,
//     rather than validating. The body
//     shape is fixed by the route's `reply` projection rather than by this
//     function, and the AngularJS client that reads `{"data":[...]}` is outside
//     this migration's scope (AAP §0.2.2).
//
// (5) Any change to the four above reaches further than GET /api/trinkets.
//     lib/controllers/folders.js `trinkets` calls this function directly for
//     GET /api/folders/{folderId}/trinkets, so a cap or a coercion change
//     applied here silently changes that route too.
// ---------------------------------------------------------------------------
function listCore(listOptions, actingUser) {
  ...
  // The five permitted sort values. Every other string reaches the lookup at
  // the `$sort` stage below and throws there rather than being rejected up
  // front, which is characteristic (3) in the block above the function.
  var sortMap = {
  ...
  // The three `$project` stages stand between the `$match` and the `$sort`, and
  // they are what makes every sort key a computed field. That ordering is the
  // mechanism behind characteristic (1) above: the sort can never be served
  // from an index, whatever indexes the collection carries.
  var pipeline = [
    { $match   : match     },
    { $project : project_1 },
    { $project : project_2 },
  ...
  // A negative `offset` is passed to mongod unchecked and mongod refuses it,
  // which is the second half of characteristic (3) above.
  if (options.offset) {
    pipeline.push({
  ...
  // The only bound on the result set, and it is a default rather than a
  // ceiling: `parseInt` of the caller's value wins whenever it is truthy, so
  // any positive value -- to the whole collection -- is honoured, a negative
  // one is refused by mongod, and 0 and every unparseable value fall back to
  // 20. Characteristics (2), (3) and (4) above all pass through this line.
  pipeline.push({
...
function downloadZip(request, h) {
  ...
  outputPromise = new Promise(function(resolve, reject) {
    // 'error' is the event archiver actually emits. This listener was written as
    // 'err' and therefore never ran, which left archiver's own 'error' with NO
    // listener at all - and an EventEmitter with no 'error' listener rethrows, so
    // the failure never reached `outputPromise` and could not be claimed by the
    // guard below.
    //
    // MEASURED, through supported routes only: the owner autosaves a `code` value
    // whose file content is an object - `[{"name":"main.py","content":{"bad":"x"}}]`,
    // which `POST /api/trinkets/{id}/autosave` accepts and persists verbatim - and any
    // ANONYMOUS `GET /python/{shortCode}.zip` then reaches `archive.append` with a
    // source that is neither Buffer nor Stream. archiver emits 'error'
    // SYNCHRONOUSLY (ArchiverError INPUTSTEAMBUFFERREQUIRED, archiver/lib/core.js),
    // node prints `throw er; // Unhandled 'error' event` and the process exits,
    // taking every concurrent in-flight request with it and orphaning the 0700
    // directory created above. Unauthenticated, repeatable, and reachable by anyone
    // who can view a shared or embedded trinket.
    //
    // Same precedence as the guard below and as AAP 0.7's decision on
    // lib/controllers/files.js:98-100: R-b's "every route serves" outranks R-d for
    // the ABSENCE of a response, because a killed process is not behaviour a client
    // can depend on, and the intended response is not inferred - it is present in
    // this same file. `downloadPostedZip` registers this identical listener on
    // 'error' and answers the identical malformed-source failure through its own
    // mapping, so correcting the event name routes this failure to the mapping this
    // route ALREADY has rather than introducing one: the rejection propagates at
    // `return outputPromise` below, into the terminal `.catch` that already calls
    // `removeZipDir()` and already answers through `legacyReply`. Every status and
    // payload this route produces for a request that produced one at all is
    // therefore unchanged (R-e); the only path that changes is the one that
  ...
  // Claimed in the same synchronous turn, for the reason spelled out at the matching
  // line in `downloadPostedZip`: nothing awaits `outputPromise` until
  // `Promise.allSettled(assetPromises)` settles, so a rejection raised before then -
  // an archive 'error' from a malformed stored file content, a write-stream error, or
  // a non-ENOENT archive warning - drains unclaimed and Node 22 turns it into a
  // process exit that takes every concurrent request with it. The
  // two zip flows are deliberately not deduplicated (AAP 0.4.1), so the guard is
  // stated twice as well. It claims the rejection and nothing more: the value still
  // reaches the `.catch` at the end of the chain, which still calls `removeZipDir()`
  // and still answers through `legacyReply`, so every status and payload on this route
  // is unchanged.
  outputPromise.catch(function() {});
  ...
  // DIRECTORY OWNERSHIP for the synchronous window, exactly as in
  // `downloadPostedZip` and for the same measured reason: `parseLegacy` on a
  // malformed STORED asset URL, and `archive.append` on a malformed stored file
  // content, both throw out of the loops below before the promise chain exists, and
  // each left the 0700 directory behind holding its partial archive. The throw is
  // re-thrown unchanged, so it still reaches the route catch-all through
  // `getByShortCode` and still answers 500 with the same payload (R-e); only the
  // leftover goes. The two zip flows are deliberately not deduplicated
  // (AAP 0.4.1), so the guard is stated in both.
  try {
    archive.pipe(outputWriteStream);
    ...
    // A malformed STORED asset URL throws ERR_INVALID_URL synchronously out of this
    // loop, before the returned chain exists. The throw still reaches the route
    // catch-all through `getByShortCode` and still answers 500, because the guard
    // enclosing this region re-throws it unchanged - it only removes the private
    // directory on the way out, which this path used to leave behind holding the
    // partial archive built before the throw.
    trinket.assets.forEach(function(asset) {
