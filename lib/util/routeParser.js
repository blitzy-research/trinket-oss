...
/**
 * Restates a declaration's `payload.output` in the hapi-17+ spelling.
 *
 * `payload : { maxBytes : N, output : 'file' }` is how the four upload routes
 * have been declared since 2013 - `POST /file` and `POST /file/avatar` in
 * config/routes.js, `POST /api/users/assets` and
 * `POST /api/users/assets/{fileId}` in config/api_routes.js - and on hapi 10
 * that one key meant "write the multipart file parts to disk". It no longer
 * does, and this is where that gap is closed, because it is an API-spelling
 * difference between framework versions and belongs in the migration layer
 * rather than in 116 hand-written declarations.
 *
 * WHAT CHANGED UNDER THE ROUTE. hapi defaults `payload.multipart` to FALSE
 * [node_modules/@hapi/hapi/lib/config.js:144-149] and @hapi/subtext answers
 * 415 Unsupported Media Type to a `multipart/form-data` body whenever it is
 * [node_modules/@hapi/subtext/lib/index.js:92-96] - from the payload parser,
 * before the handler exists and before validation runs. Every shipped client
 * posts multipart (public/js/courseEditor/controllers/materialControl.js:157,
 * lib/views/users/includes/profile.html:88-94,
 * public/js/plugins/asset-browser.js:270-271,528-530), so left untranslated
 * no upload can succeed on any of the four routes: no File document and no
 * stored object is creatable through the application.
 *
 * WHY `multipart.output` AND NOT THE TOP-LEVEL `output`. Subtext reads the
 * per-part output from `options.multipart.output` and ignores the top-level
 * `output` for multipart bodies [.../subtext/lib/index.js:290], so file parts
 * are still written to disk exactly as they were. Keeping the top-level
 * `output` as well would additionally spool every NON-multipart body to a temp
 * file and hand the handler `{path, bytes}` - a shape nothing here reads,
 * since the handlers consume `request.payload.upload` - while the
 * validation-failure funnel below echoes the rejected payload back through
 * `request.fail(request.payload, ...)`, which would disclose an absolute
 * server filesystem path at status 200. So `output` moves rather than being
 * duplicated. `maxBytes` is untouched and still applies to both body classes;
 * subtext checks it against the content length before any parsing
 * [.../subtext/lib/index.js:41].
 *
 * A declaration that already states `multipart` has said what it wants, and
 * nothing here overwrites it. The rewrite is idempotent: after it runs there
 * is no top-level `output` left to match, so a second parse of the same
 * objects is a no-op.
 *
 * @param {Object} options A route's `options` (the renamed `config`) block.
 * @returns {undefined} `options.payload` is rewritten in place, which is the
 *   same in-place contract every other mutation in `parseRoutes` follows.
 */
function migratePayloadOutput(options) {
  ...
  if (payload.multipart === undefined) {
    payload.multipart = { output : payload.output };
  }
...
function parseRoutes(routeConfigs) {
  ...
  routeConfigs.forEach(function(route, index) {
    ...
    if (route.options) {
      ...
      // `payload.output : 'file'` is the pre-hapi-17 spelling of
      // `payload.multipart.output`; see migratePayloadOutput above.
      migratePayloadOutput(route.options);
