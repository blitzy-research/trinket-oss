...
module.exports = {
  ...
  create : async function(request, h) {
    // recaptcha.verify resolves a single result object whose failure shape is { status : false }, key
    // `status` and never `success`, so the `.success` read below is falsy on a genuine failure by
    // accident rather than by design. Async conversion: the module is promise-native, so the base
    // commit's own `new Promise` bridge is gone and this awaits it directly.
    ...
    // PRESERVED QUIRK (docs/PRESERVED-QUIRKS.md section 4.4). Both halves of this destination are
    // user-controlled and neither is constrained to this application's own origin: the session value
    // is set by `GET /signup?next=…` and `payload.next` is a declared payload field -
    // config/routes.js:L88 accepts it as `Joi.string().allow('').optional()`, so a caller may supply
    // it even though no shipped template renders it. It reaches the responder below as `redirectTo`,
    // which lib/http/redirect.js absolutizes into a Location header, leaving an already-absolute
    // off-origin value untouched. That is the base commit's behavior, kept under R-4 and R-6.
    ...
    try {
      ...
      // Async conversion: lib/models/user.js#exists is promise-native, so the base commit's
      // `new Promise` bridge (base lib/controllers/users.js:L62) is gone. It resolves the same
      // `{exists, duplicates, users}` object the callback received and rejects with the same mongoose
      // error, which the surrounding catch already handles. `exists` is pre-bound to its model by
      // lib/models/model.js:L176, so no .call is needed.
      ...
      // Log in the user
      // Async conversion: _logIn is a SYNCHRONOUS decoration that takes no callback
      // (app.js:L113-L124). The base commit passed one and awaited a bridge around it, whose reject
      // arm was unreachable because the error argument was always null. A direct call is exactly
      // equivalent, and it keeps the session write ordered ahead of the response as before (TR4).
  ...
  login : async function(request, h) {
    ...
    // PRESERVED QUIRK (docs/PRESERVED-QUIRKS.md section 4.4). The session's `next` originates from
    // the `GET /login?next=…` query string and is emitted VERBATIM as a Location header at the
    // toolkit redirect below - not absolutized, not confined to this origin - so this login form
    // answers a Location the visitor's own query string chose. Both the absolute same-origin
    // destination the frozen assignment UI sends (which
    // test/baseline/responses.json#assignmentNext measures) and an off-origin one travel that same
    // path. It is reproduced rather than repaired: R-4 forbids behavior improvements, R-6 makes the
    // base commit the tie-breaker.
    ...
    try {
      ...
      // Async conversion: _logIn is a synchronous decoration with no callback (app.js:L113-L124); the
      // base commit's empty callback went with the parameter.
      ...
      // This is why the site is NOT routed through NoResponse.rejectOrAbandon - unlike the three orphaned
      // request.fail(<Error>) sites in sendPassReset, assetUpload and assetUploadFromURL, whose raises
      // could never reach a response at all. See lib/http/responseContract.js.
      return h.reject(err);
    }
  },
  ...
  sendPassReset : async function(request, h) {
    ...
    if (result.success) {
      ...
      // PRESERVED QUIRK - see docs/PRESERVED-QUIRKS.md, and the derivation in lib/http/responseContract.js.
      ...
      // invent a 500 instead. NoResponse.rejectOrAbandon reproduces the measured fate and is transparent
      ...
      // reproduced instead. See lib/http/responseContract.js for the derivation. A failure of the SECOND write
      ...
      // Fire-and-forget, deliberately un-awaited: the response above is returned without waiting
      // for SMTP. The terminal catch owns the rejection without changing anything observable
      // (review finding F7) - a reset mail failure was never reported to the caller and still is
      // not, but an unowned rejection is process-fatal under Node 22, which would turn one
      // unreachable SMTP host into a downed server on the password-reset route.
  ...
  updateProfile : async function(request, h) {
    ...
    if (user.username !== payload.username.toLowerCase()) {
      // Async conversion: lib/models/user.js#exists is promise-native, so the base commit's own
      // bridge (base lib/controllers/users.js:L385) is gone. Calling it HERE rather than inside a
      // later .then keeps the lookup EAGER exactly as baseline: the query is issued before the
      // `user.set(...)` below, and only awaited further down.
  ...
  assetUpload : async function(request, h) {
    ...
    // capture. lib/util/file.js#uploadUserAsset is now promise-native, so no bridge is needed; the
    // two-handler .then still captures both outcomes so the branch below keeps the failure at HTTP
    // 200. Note the TWO-argument call: uploadUserAsset accepts both this arity and the
    // three-argument replace form used by `replaceAsset`, and each site's arity is preserved. Its
    // rejection carries the same error object the base commit's `cb(err, file)` delivered as its
    // first argument; the `file` that accompanied it on the upload-failure branch was already
    // discarded by the util.promisify bridge and no branch here reads it.
    var upload = await FileUtil.uploadUserAsset(request.payload.file, request.user).then(
      function(file) { return { file : file }; },
      function(err)  { return { err : err }; }
    );

    // PRESERVED QUIRK - see docs/PRESERVED-QUIRKS.md and the derivation in lib/http/responseContract.js. The
    ...
    // raise never became a response and the request answered NOTHING; NoResponse.rejectOrAbandon preserves
  ...
  assetUploadFromURL : async function(request, h) {
    ...
    if (!download.completed) {
      ...
      // deferred capture is gone"; it is reproducible - `lib/http/responseContract.js` is exactly that - and
    ...
    // Async conversion: the TWO-argument upload form, awaited exactly as in `assetUpload`.
    var upload = await FileUtil.uploadUserAsset(fileupload, request.user).then(
      function(file) { return { file : file }; },
      function(err)  { return { err : err }; }
    );

    // PRESERVED QUIRK - see docs/PRESERVED-QUIRKS.md and lib/http/responseContract.js. Error as the
  ...
  activateAccount : async function(request, h) {
    ...
    try {
      ...
      // is a SYNCHRONOUS decoration that takes no callback (app.js:L113-L124), so the bridge is
      // replaced by a direct call - which also keeps baseline's ignoring of the null error argument.
      request.yar._logIn(user);
  ...
  requestExport : async function(request, h) {
    ...
    return Export.findPendingOrProcessing(userId)
      ...
      .then(function(saved) {
        ...
        //
        // The terminal catch is OWNERSHIP, not a behaviour change (review finding F4). bull 4.16.5's
        // add() returns a promise - measured - so the bare call left a rejection with no handler, and
        // Node 22's default `--unhandled-rejections=throw` turns that into a process-fatal fault: an
        // unreachable Redis would take the whole server down instead of leaving one export pending. The
        // catch is empty because the response, the persisted `status: 'pending'` record and the
        // message below must all stay exactly as they are - a queue failure leaves the export pending
        // forever, which is precisely what it did before.
        exportsQueue.add({
          action: 'bulk-export',
  ...
  downloadExport : async function(request, h) {
    ...
    // `throw Boom.x(...)` statement byte-for-byte and answers with `h.abandon` from one catch.
    ...
    // "unavoidable"; it is avoidable, `lib/http/responseContract.js` is exactly that, and R-4 forbids the
...
function send_email_confirmation(request, new_email, key) {
  ...
  // Both helpers stay synchronous and both sends stay un-awaited: their callers at the change-email
  // and verify-email handlers invoke them for their side effect and answer without waiting, and
  // making either one async would move the response behind SMTP. The terminal catch is the whole
  // change (review finding F7) - it keeps a rejection from a configured transport from becoming the
  // process-fatal unhandled rejection Node 22 raises, and reports nothing, exactly as before.
