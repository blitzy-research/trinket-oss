...
// The repeat-collapsing bound on the queue-level error log, and why it exists.
//
// A queue-level error is not necessarily one event. An unreachable Redis
// produces one PER RECONNECT ATTEMPT, and Bull's own retry strategy attempts
// forever - measured on this tree as 30 identical `connect ECONNREFUSED` events
// in the first 120 seconds and a steady two every 20 seconds thereafter, which
// is some 8,600 lines a day, for as long as the process lives, all saying the
// same thing. The base commit's transport self-throttled instead: node_redis
// backed its attempts off by 1.7x, so the SAME outage reported itself
// decreasingly often. Bounding the transport's retry was measured and rejected
// (the reasoning is with the outage constants in lib/util/queues.js: a terminal
// give-up kills the worker), so the decay is reproduced HERE, where the line is
// written, by reporting an unbroken run of identical errors on a doubling
// interval.
//
// The first occurrence is always logged immediately and in full, and no repeat
// is discarded - each summary carries how many arrived and over what interval -
// so the outage stays visible at both ends and nothing is lost but the
// repetition.
//
//   FIRST      the interval before the first summary of a run: short enough
//              that an operator watching a live outage sees it confirmed while
//              they are still looking.
//   CAP        the ceiling the interval doubles to. An hour is node_redis's own
//              `connect_timeout` default, which is what bounded this log at the
//              base commit, so it is the interval at which a very long outage
//              settles into reporting itself.
//
// The resulting shape, for one unbroken run: a line at 0s, then at 60s, 180s,
// 420s, 900s, 1860s, 3780s and hourly after that - 7 lines in the first hour,
// about 30 in a day, against 8,600 unbounded.
var QUEUE_ERROR_SUMMARY_FIRST_MS = 60000;
var QUEUE_ERROR_SUMMARY_CAP_MS   = 3600000;

// The collapsing state. Deliberately NOT a timer: this file's teardown is
// asserted to leave no handle behind, so the interval is measured against the
// arriving event's own clock instead of a `setInterval` that would have to be
// cleared on close.
//
//   key         the message of the run's error, which is what makes two errors
//               "the same" here. Bull's own errors carry the job id in the
//               message - `Missing lock for job 42 failed` - so two different
//               jobs are two different runs and each is reported in full.
...
var queueErrorRepeat = {
  key        : null,
  count      : 0,
  reportedAt : 0,
  intervalMs : QUEUE_ERROR_SUMMARY_FIRST_MS
};

/**
 * The identity of one queue-level error, for repeat detection.
 *
 * The message and not the object: two ECONNREFUSED errors from two of Bull's
 * three clients are distinct objects carrying the same text, and it is the text
 * an operator reads. A non-Error argument is stringified rather than dropped,
 * because a queue-level `error` payload is whatever the transport emitted.
 *
 * @param {*} err The error the queue emitted.
 * @returns {string}
 */
function queueErrorKey(err) {
 ...
 *
 * The `exports queue error:` prefix is kept: it is the prefix every consumer of
 * this log filters on, and a summary that did not carry it would be invisible
 * to anything watching for queue trouble.
 *
 * The interval doubles on every summary, up to the cap, which is what makes an
 * outage that does not end report itself less and less often instead of at a
 * fixed rate for ever. `grow` is false when the run is ending - a DIFFERENT
 * error has arrived - because there is no next summary for that run to space
 * out.
 *
 * @param {number} now Millisecond clock reading for this write.
 * @param {boolean} grow Whether to widen the interval for the next summary.
 * @returns {undefined}
 */
function flushQueueErrorRepeats(now, grow) {
  if (queueErrorRepeat.count === 0) {
    return;
  }

  // Widened BEFORE the line is written, so the interval the line quotes is the
  // one that will actually apply to the next summary rather than the one that
  // has just elapsed.
  if (grow) {
    queueErrorRepeat.intervalMs = Math.min(queueErrorRepeat.intervalMs * 2,
      QUEUE_ERROR_SUMMARY_CAP_MS);
  }

  console.log('exports queue error: same error repeated ' +
    queueErrorRepeat.count + ' more time(s) in the last ' +
    Math.round((now - queueErrorRepeat.reportedAt) / 1000) + 's' +
    (grow ? ', reported again in at most ' +
      Math.round(queueErrorRepeat.intervalMs / 1000) + 's while it lasts' : '') +
    ': ' + queueErrorRepeat.key);

  queueErrorRepeat.count      = 0;
  queueErrorRepeat.reportedAt = now;
}
...
//
// The FIRST occurrence of each distinct error is logged exactly as it always
// was, error object and all, so the stack survives; only an identical repeat
// within the summary window is held back, and it is held back as a count rather
// than discarded.
exportsQueue.on('error', function(err) {
  var key = queueErrorKey(err);
  var now = Date.now();

  if (key !== queueErrorRepeat.key) {
    // A different error ends the previous run: report what it accumulated
    // before starting a new one, or the count would be silently lost. The
    // interval is not widened for a run that is over, and it resets with the
    // new run so a fresh problem is never reported at the previous one's
    // stretched cadence.
    flushQueueErrorRepeats(now, false);

    queueErrorRepeat.key        = key;
    queueErrorRepeat.count      = 0;
    queueErrorRepeat.reportedAt = now;
    queueErrorRepeat.intervalMs = QUEUE_ERROR_SUMMARY_FIRST_MS;
  ...
  queueErrorRepeat.count++;

  if (now - queueErrorRepeat.reportedAt >= queueErrorRepeat.intervalMs) {
    flushQueueErrorRepeats(now, true);
  }
});
