...
function createModel(modelName, config) {
  ...
  // invokes that callback inside a try/catch and turns anything it throws into
  // `model.emit('error', err)`; `Query.prototype.exec` routes every callback
  // through it [node_modules/mongoose/lib/query.js:4935]. An 'error' event with
  // no listener is rethrown by EventEmitter, so ONE controller callback that
  // throws would terminate the whole process and take every concurrent
  // in-flight request with it. Measured: a single `GET /api/exports/{exportId}`
  // for an absent document reaches the unbound `Boom` in
  // lib/controllers/users.js, whose ReferenceError escaped the callback and
  // killed the server mid-run; the parity replay stopped there with 131
  // scenarios never driven.
  //
  // This listener is now the SECOND line of that defence rather than the only
  // one. `findById`'s callbacks are re-delivered their own throw immediately
  // below, exactly as the base commit's `.catch(cb)` did, so the edge that threw
  // answers its request; what reaches this listener is a throw from that second
  // invocation, which at the base commit rejected the finder's promise chain
  // with no handler attached and ended the process on Node 22's unhandled
  // rejection policy. It also still catches a throw from every callback the
  // other finders hand over unchanged. It logs and stops there: it does not
  // answer the request, retry the query or reshape any response.
  ...
  model.$handleCallbackError = function(callback) {
    ...
    reDelivering = function() {
      try {
        ...
        // here - it propagates into Mongoose's wrapper, which emits it as the
        // model 'error' the listener above logs. That bounds the re-delivery at
        // one and cannot loop.
        callback(thrown);
      }
    };
