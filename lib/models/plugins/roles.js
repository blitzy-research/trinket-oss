...
module.exports = function(schema) {
  ...
  schema.methods.hasRole = function() {
    var args = Array.prototype.slice.call(arguments)
      , equivalents, candidate, i;

    args.unshift('roles');

    if (has.apply(this, args)) {
      return true;
    }

    // a legacy role name is also satisfied by any role declared equivalent to
    // it in lib/models/roles.js, so an account that only ever had 'user'
    // granted still reports the legacy name whose permissions it already has.
    // args[1] is the requested name; every other argument - context, and
    // options.id / options.extension / options.extId - is passed through
    // unchanged, so an equivalent is tested under exactly the same context,
    // instance id and extension as the name the caller asked for, including
    // that name's own thru and limit handling inside has().
    equivalents = Roles.getEquivalentRoles(args[1]);

    for (i = 0; i < equivalents.length; i++) {
      candidate    = args.slice();
      candidate[1] = equivalents[i];

      if (has.apply(this, candidate)) {
        return true;
      }
    }
