...
//
// obs-login-enumeration-no-throttle: `login` below answers four
...
// "Account Disabled" - immediately and, before this block existed, without
// limit. The first three are a credential oracle (CWE-204): they tell an
// unauthenticated caller whether an account exists and what state it is in,
// and the first one echoes the submitted identifier back into the response.
//
// THE MESSAGE HALF OF THAT FIX HAS BEEN WITHDRAWN, ON INSTRUCTION.
// An earlier round collapsed the first three into one generic string. QA
// finding F24 ("Invalid-login message changed outside approved deviations")
// raises that collapse as an unauthorised deviation from base commit 2f8712a,
// and AAP rule R-d makes baseline's text the contract, so all four messages
// are back verbatim at their branches below
// [git show 2f8712a:lib/controllers/users.js:121,125,129,144]. The credential
// oracle is therefore open again, deliberately and by instruction, and is
// recorded in the resolution report rather than left to be rediscovered.
//
// THE RATE HALF STAYS. No finding in this checkpoint raises it, it changes no
// status, header or response body - only timing - and withdrawing a closed
// remediation re-opens the round that closed it. It remains the only limit on
// a password spray against a known account.
//
// This block is the rate half of that finding: 20 consecutive wrong-password
// attempts used to be answered 200 with no delay, lockout or CAPTCHA at all.
//
...
//     would widen that diff; with the four baseline messages restored the
//     throttle leaves the response byte-identical to baseline and is visible
//     only in its timing.
//
...
// caller lands on another worker is still a delay.
// ---------------------------------------------------------------------------

var LOGIN_FAILURE_FREE_ATTEMPTS   = 3,
...
module.exports = {
  ...
  login : async function(request, h) {
    ...
    try {
      ...
      if (!user) {
        ...
        // QA F24: the base commit answers `'Unknown user ' + requested` here
        // [git show 2f8712a:lib/controllers/users.js:121], and this checkpoint's
        // report raises the shared generic message as an unauthorised deviation.
        // Rule R-d makes baseline's text the contract, so it is restored
        // verbatim, identifier interpolation included. The exponential backoff
        // below stays: no finding raises it, it changes no response body, and
        // withdrawing it would re-open the round that introduced it. The
        // enumeration oracle the generic message closed is therefore back, by
        // instruction, and is recorded as such rather than silently traded away.
      ...
      if (!user.password || user.password.length === 0) {
        // Baseline text restored for QA F24
        // [git show 2f8712a:lib/controllers/users.js:129].
        await throttleLoginFailure(throttleKey);
      ...
      if (!isMatch) {
        // Baseline text restored for QA F24
        // [git show 2f8712a:lib/controllers/users.js:144]. The backoff is the
        // branch it matters most for and stays, as above.
        await throttleLoginFailure(throttleKey);
  ...
  // APPROVED DEVIATION -- this handler used to answer only on its
  // duplicate-address branch, and now answers on both.
  //
  // THE DEFECT. The store write below was passed a completion callback as a
  // THIRD argument, and `Store.set` is an arity-2 `async function (key, val)`
  // [lib/util/store.js, unchanged from 2f8712a], so the argument was ignored
  // and the callback never ran. Measured on BOTH trees, identically: POST
  // /api/users/email with a new address answers NOTHING (curl -m 15 gives up
  // at 15.00s with status 000) while the pending change IS written - the very
  // next `GET /api/users/resendEmailChange` answers 200 {"success":true} from
  // that stored value - and no confirmation mail is sent, so the address the
  // visitor asked for is left pending with no way to confirm it. The
  // duplicate-address branch answers 200 in 8ms on both trees.
  //
  // WHY IT IS FIXED RATHER THAN PRESERVED. R-d protects observable behaviour a
  // client may rely on, and AAP 0.7 decides precisely this collision for
  // lib/controllers/files.js:98-100: "an unsettled request is not a behaviour
  // a client can depend on - it is the absence of a response", and R-b's
  // requirement that every route serve controls. The two cases match on all
  // three of that decision's grounds: the request is never answered rather
  // than answered differently; the response the branch intended is present in
  // the SAME function, on the sibling branch and on `resendEmailChange`
  // immediately below, which produces `request.success({success: true})` from
  // the same stored record; and R-b is unqualified. Neither AAP 0.6.6's
  // preserved-quirk catalogue nor 0.4.1/0.4.2's per-site notes for this file
  // name this non-settlement, so no explicit mandate preserves it the way AAP
  // 0.4.2 mandates the streaming fetch's (see assetUploadFromURL above, which
  // is left unsettled for exactly that reason).
  //
  // WHAT MOVES, AND WHAT DOES NOT. Only the dead callback becomes the promise
  // the store actually returns. The stored key and value, the confirmation
  // mail, the un-awaited hand-off to the mail transport, the success payload
  // and the duplicate-address branch are all unchanged, and the write is still
  // the only thing the response waits on. A store rejection - which reached
  // nothing at all before, since nobody held that promise - is routed into
  // `request.fail`, the funnel this handler already uses for its other
  // branch, so the route no longer has an edge that answers nothing.
  //
  // The committed parity artifacts record the old outcome: the corpus carries
  // this route with `intent: "timeout"` and test/parity/joi-matrix.js's
  // reviewed-timeout register names this call as the reason. Both are
  // regenerated with the deviation recorded, which is what AAP 0.7 requires of
  // an approved deviation - the artifact says what the target does and why it
  ...
  sendEmailChange : async function(request, h) {
    ...
    // The promise boundary is created here, at the lifecycle method, and every
    // terminal branch resolves it with the response that branch produces.
    return await new Promise(function(resolve) {
      User.findByLogin(request.payload.email, function(err, user) {
        ...
        require('crypto').randomBytes(48, function(ex, buf) {
          ...
          // `Store.set` returns a promise and takes no callback, so the write
          // is awaited through the value it actually produces. The order is the
          // one the dead callback described: the record is written first, then
          // the confirmation mail is handed off un-awaited (through
          // send_mail_detached, so a transport rejection is observed rather
          // than terminating the process), then the response is settled.
          Store.set(changeKey, JSON.stringify(changeVal))
            .then(function() {
              ...
              // The store is the only thing this branch waits on, so its
              // failure is the branch's one error edge. It answers through
              // `request.fail` - the funnel the duplicate-address branch above
              // already uses, and the one AAP 0.6.3 calls Layer 2 - with a
              // fixed message in the style of every other request.fail in this
              // module, and with the error itself as the second argument,
              // which is what puts it in the log line without putting the
              // store's internals in the response. No record was written and
              // no mail was sent on this path, so the pending change the
              // visitor asked for simply did not happen.
              resolve(request.fail({
                message : 'Something went wrong when trying to change your email address. Please try again.'
              }, storeError));
            });
        });
      });
