...
/**
 * Every host[:port] that is this application's own, normalized under `protocol`.
 *
 * Three sources, all of them configuration or the request itself - never the
 * candidate destination:
 *   1. config.url, the origin lib/http/redirect.js absolutizes every declarative
 *      redirect onto, so a target already carrying it is the same destination;
 *   2. config.app.url.hostname[:port], the same origin read straight from
 *      configuration, which also covers a caller that loaded this module without
 *      config/app.config.js having assembled config.url;
 *   3. request.info.host - the Host the client itself addressed. Including it is
 *      what makes a deployment whose config.app.url differs from the address in
 *      use (localhost:3000 in development, an ephemeral port under supertest)
 *      behave the same as production. It cannot be abused: an attacker cannot set
 *      a victim's Host header, and a Location back to the host the client already
 *      addressed is by definition not an off-origin redirect.
 *
 * @param   {String} protocol   The candidate's protocol, including the trailing colon.
 ...
 *   2. An absolute http(s) URL whose parsed origin is one of this application's own
 ...
 * payloads, and bare relative values ('courses/x') that would resolve against
 * whatever path the browser is currently on.
 *
