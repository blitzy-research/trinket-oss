...
module.exports = {
  ...
  create : async function(request, h) {
    try {
      ...
      //
      // That outcome is reproduced rather than converged: `h.abandon` is hapi's own no-response
      // outcome, so this one request is left unanswered while every other route is served normally.
      // Falling through instead would resolve `undefined` and make hapi answer 500 - a status this
      // branch never produced. No else branch, default response or guard is added, because inventing
      // one would answer requests that no baseline request ever answered. See
      // lib/http/responseContract.js for the measured semantics.
      return h.abandon;
    } catch (err) {
  ...
  copy : async function(request, h) {
    ...
    // Async conversion: Course#copy is promise-native, so this awaits it directly - no util.promisify
    // and no hand-written bridge. It is called as a method, so `this` is the course document the
    // method reads `this.name`, `this.description` and `this.globalSettings` from.
    //
    ...
    // asks for the no-response fate rather than for the crash. The observable per-request outcome -
    // the client never receives a response - is identical.
    //
    // There is deliberately no timeout, and none may be added. Course#copy has TWO failure channels
    // and both answer nothing here: its first save reports a copy failure through `cb(err)`, and its
    // inner lesson/material chain - which reached cb not at all at the base commit - now settles with
    // the silent-outcome sentinel (review finding F6; lib/models/model.js#silentOutcome and
    // docs/PRESERVED-QUIRKS.md section 3.40). This handler answers BOTH channels identically - unlike
    // lib/controllers/course.js#copyCourse, which has a duplicate-name branch to keep them apart - so
    // the one catch below is the whole translation and no per-channel branch is added.
    try {
