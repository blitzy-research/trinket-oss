...
var util             = require('util'),
    ...
    // Only for isSecretField(). lib/http/responseContract.js#reject logs the `err` string
    // built below, and Joi embeds the OFFENDING VALUE in its pattern/min/max messages, so a
    // rejected password would otherwise reach the log in cleartext. isSecretField() is the
    // responder's own definition of a secret-bearing name, taken from there rather than
    // duplicated so the two cannot drift. LOG ONLY - the map that is flashed and re-rendered
    // is `validationErrors`, untouched. See docs/PRESERVED-QUIRKS.md section 15.6.
    ResponseContract = require('./responseContract');

function validate(request, h, validation, language, reject) {
  var validationErrors, loggableErrors;
  ...
  if (validation) {
    ...
    if (validationErrors) {
      ...
      // re-rendered, which is the frozen behavior, and the responder redacts it only on the
      // way to the log. The second argument is log-only, so it is pre-scrubbed here. Every
      // byte the client receives is the base commit's; see docs/PRESERVED-QUIRKS.md
      // section 15.6 for why this remediation is wire-neutral and therefore kept.
