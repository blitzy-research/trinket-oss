  ...
  routeConfigs.forEach(function(route, index) {
    ...
    route.handler = async function(request, h) {
      ...
      try {
        ...
        // Handlers converted to the native idiom address their second argument as the response
        // toolkit and call `h.redirect(...)` / `h.response(...)` / `h.view(...)` on it, while the
        // handlers still awaiting conversion call it as the legacy `reply(data)` function. Until the
        // compatibility layer above is retired, both calling conventions have to be served by the
        // same value, so the legacy `reply` function is presented with the real toolkit behind it:
        // calling it keeps the builder semantics, while any member it does not define itself falls
        // through to `h`. The fall-through is a read-only delegation, so it is mapping-neutral - the
        // builder's `redirect` already returned exactly `h.redirect(url)`, and it also picks up
        // toolkit members added by plugin decoration such as Vision's `view`.
        //
        // NOTE this does NOT resurrect the property-form redirect quirk that authenticated
        // GET /login and GET /signup depend on: those two handlers raise Boom.badImplementation
        // explicitly rather than relying on `redirect` being absent. See docs/PRESERVED-QUIRKS.md.
        var toolkit = new Proxy(reply, {
          get : function(target, prop, receiver) {
            ...
            var member = h[prop];
            return typeof member === 'function' ? member.bind(h) : member;
          }
        });
        ...
        if (handler) {
          ...
          var result = await handler.call(this, request, toolkit);
