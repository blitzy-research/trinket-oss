// Legacy-compatible URL parsing, with no deprecation warning anywhere on the
// path and nothing suppressed to achieve that.
//
// WHY THIS MODULE EXISTS
// ---------------------------------------------------------------------------
// Six sites parse a stored asset URL or a user-supplied URL and read one or
// two fields off the result. They are named by symbol rather than by line,
// because all three files are edited by other work and any line number written
// here goes stale:
//
//   * `users.assetUploadFromURL` reads `protocol` twice - once for the reject
//     guard and once for the http/https transport gate - and `path` for the
//     upload filename.
//   * `trinket.downloadFile` reads `pathname` in its asset branch, and so do
//     `trinket.downloadPostedZip` and the module-private `downloadZip` in
//     their asset loops - three sites, each feeding `path.basename`.
//   * `exports.downloadAsset` and `exports.addTrinketToArchive` each read
//     `pathname`.
//
// Every one of them used `url.parse()`, which carries two deprecation
// warnings: DEP0169 on every call when the process runs under
// `--pending-deprecation`, and DEP0170 from inside the parser for a malformed
// authority, unconditionally and regardless of that flag. Both are reachable
// with a URL a user supplies, which is what put a deprecation warning on the
// stderr of a normal production process and breached the whole-application
// zero-deprecation contract. This module is the single shared replacement.
//
...
//     `protocol` is null. `users.assetUploadFromURL` reads `.protocol` off
//     that object and answers with `request.fail()`, so returning null instead
//     would throw a TypeError there and turn a redirect into a 500 - and would
//     break the five asset sites, which read `.pathname` with no null check.
//   * It does not resolve against a base. Resolving '//h/x', '/x' or 'x'
//     would give them a protocol, and that same guard would stop rejecting the
//     relative URLs it rejects today.
//   * It keeps the query string on `.path` while leaving it off `.pathname`,
//     and `users.assetUploadFromURL` derives the stored upload filename with
...
//
// Measured on Node 22.23.2, and reproduced field for field by this module:
//
...
// `TypeError [ERR_INVALID_ARG_TYPE]`. Both errors are constructed here with
// the shape Node's own errors carry: `name` 'TypeError' inherited rather than
// own, an enumerable `code` (and `input`, for the URL error), a
// non-enumerable `message`, a stack header that names the code, and a
// `toString()` of the form 'TypeError [CODE]: message'.
//
// HOW PARITY IS GUARANTEED, AND HOW THE WARNING IS REMOVED
// ---------------------------------------------------------------------------
// The parser below is a faithful transcription of Node 22.23.2's own legacy
// parser - `Url.prototype.parse`, `getHostname`, `Url.prototype.parseHost`,
// `autoEscapeStr` and their module-level tables, translated line for line into
// this file's ES5 CommonJS style. It is not an approximation of the legacy
// semantics: those semantics are idiosyncratic in ways a reimplementation
// would not reproduce by accident (an unusable port folds into the path, a
// space ends the host, a backslash becomes a slash, a tab is escaped, and the
// throwing set has near neighbours that do not throw), so the algorithm is
// carried over rather than re-derived.
//
// The single DEP0170 emission site lives in `getHostname`, which warns when an
// authority contains a colon that cannot begin a usable port. In the
// transcription below **that site is simply absent**: `getHostname` keeps its
// two behavioural lines - it truncates the hostname at the offending character
// and folds the remainder into the path - and neither the `warnInvalidPort`
// one-shot flag nor the warning call it guards is ported at all.
//
// The consequence is the point of this module, and it is stronger than "the
// warning does not appear": nothing is invoked and nothing is suppressed, so
// no process-global warning state is touched. Node's process-wide warning
// emitter is never replaced, its one-shot DEP0170 flag is never spent, and a later
// `url.parse()` of a malformed authority anywhere else in the process - in a
// dependency, in a script, in a test - still warns exactly as it would if this
// module had never been loaded. That is what the previous revision got wrong:
// it consumed the flag at load time, which removed the warning and, with it,
// the whole process's ability to report DEP0170 from anywhere else.
//
// THREE SUBSTITUTIONS, EACH MEASURED WARNING-FREE
// ---------------------------------------------------------------------------
// Three things the built-in parser reaches through internal bindings cannot be
// required from application code. Each is substituted, and each substitution
// was measured to emit nothing under `--pending-deprecation
// --trace-deprecation`:
//
//   * `href` - Node ends its parse with `this.href = this.format()`. This
//     module populates a real `Url` instance (the exported constructor only,
//     never its `parse` method) and calls `Url.prototype.format` on it, which
//     is what Node does. `format` carries no deprecation and no warning path.
//     Returning a genuine `Url` instance also keeps `instanceof url.Url` true
//     for callers and for this module's own short-circuit.
//   * `querystring` - reachable only when `parseQueryString` is true, which no
//     caller sets. The branch is transcribed against the public
//     `querystring` module rather than dropped, so the transcription stays
//     complete.
//   * IDNA - Node's parse calls the internal `toASCII` from the
//     `encoding_binding`, which has no public equivalent. `toASCII()` below
//     reconstructs it from `url.domainToASCII`, and the reconstruction is not
//     cosmetic: `domainToASCII` is the *WHATWG host parser*, and used directly
//     it diverges from the legacy IDNA on 24 of 127 measured hosts. It
//     canonicalises IPv4-shaped hosts ('0x7f.1' -> '127.0.0.1', '1.2.3' ->
//     '1.2.0.3', '0' -> '0.0.0.0'), it *rejects* others outright
//     ('999.999.999.999', '256.1.1.1' - and 'http://999.999.999.999/a' is one
//     of the near neighbours that must not throw), it percent-decodes its
//     input, and it rejects C0 controls and DEL that legacy IDNA keeps. Four
//     corrections close every one of those gaps - an identity path for hosts
//     that need no IDNA at all, a sentinel label that suppresses the IPv4
//     branch, a bidi-class-matched mask for the code points WHATWG refuses,
//     and one directly-taken verdict for the rule a mask cannot carry - and
//     each is documented on the function that applies it. Both halves are held
//     to the runtime's own answer by the parity corpus rather than by
//     argument: 18,435 inputs, 235,021 field and error comparisons, zero
//     divergences.
//
// MAINTENANCE
// ---------------------------------------------------------------------------
// This file is a vendored copy of one runtime's algorithm - Node 22.23.2, the
// version `.nvmrc` and `package.json` `engines` pin. A newer runtime can
// change legacy parsing (DEP0170 exists precisely because the unusable-port
// leniency is meant to become an error), and when it does, this copy will
// silently be the old behaviour rather than fail loudly. What keeps it honest
// is not review but the oracle parity test: the parity matrix in
// test/lib/api/trinket.js compares `parseLegacy` against the running
// runtime's own `url.parse` on all twelve legacy fields and on the error
// identity, so a runtime that changes the algorithm fails that suite instead
// of quietly changing what the six consumers see. Re-derive this file from
// `node --no-warnings -e "process.binding('natives').url"` on the new
// runtime when that happens.
//
// WHY NOT ONE OF THE ALTERNATIVES
// ---------------------------------------------------------------------------
// Recorded here because each is a plausible-looking simplification that would
// break a caller:
//
//   * Not `Url.prototype.parse`, driven directly or with the warning
//     suppressed. Suppression is what this revision replaces: it required
//     replacing the process-wide warning emitter or spending Node's one-shot
//     flag, and either way the deprecated code path is still the one that runs.
//   * Not `new URL`, with or without a base. It rejects the relative inputs
//     that must yield partial objects, and a base would give '//h/x', '/x' and
//     'x' a protocol, so lib/controllers/users.js would stop rejecting them.
//   * Not `url.domainToASCII` on its own, for the reasons measured above.
//   * Not a `null` return on failure and no `try`/`catch` here. users.js reads
//     `.protocol` off the result and the five asset sites read `.pathname`
//     with no null check, so a `null` would turn a 302 into a 500; and each
//     throw must reach the error handler it reaches today, unchanged.
//   * Not a new dependency. `punycode` is runtime-deprecated on Node 22 and a
//     userland IDNA package would be a second, differently-behaved parser.
...
function invalidUrlError(input) {
  ...
  // Node's error classes create the error, then set `code`, then let the
  // message builder set `input`, then define `message`. The own-property order
  // is part of what an inspected error looks like, so it is preserved.
 ...
 * IDNA moved them.
 *
 * The result is checked, not assumed: the two runs must agree in length, must
 * differ in exactly as many positions as there were masked code points, and
 * must carry the two placeholders at those positions. Three placeholder pairs
 * are tried. If none reconciles - a case the oracle parity corpus never
 * produced - the host is reported as an IDNA failure, which is the disposition
 * the WHATWG entry point gives it in any case.
 *
 * @param {string} domain - the host to run IDNA over
 ...
 *     with UseSTD3ASCIIRules disabled - which is the configuration legacy
 *     parsing uses - maps every ASCII code point to itself apart from the
 *     upper-case letters, and only an ACE label needs decoding and
 *     re-validation. Measured over the ASCII half of the host corpus, this
 *     path matches the runtime's own `toASCII` on every input, and it is what
 *     keeps '999.999.999.999', '0x7f.1', 'a..b', 'a\x01b' and every other
 *     shape the WHATWG parser rewrites or rejects behaving exactly as they do
 *     today.
...
/**
 * Node's `getHostname`, with its DEP0170 emission site omitted.
 *
 * The loop stops at the first character that cannot appear in a hostname. Node
 * warns at that point when the character is a colon, because it represents an
 * unusable port that `url.parse()` is lenient about - the leniency a future
 * runtime is meant to replace with an error. The two behavioural lines are
 * kept: the hostname is truncated before the offending character and the
 * remainder is folded into the path, which is why 'http://host:abc/a' has the
 * pathname '/:abc/a'. Neither the one-shot flag that guarded the warning nor
 * the warning call itself is present, so nothing is emitted and nothing has to
 * be suppressed.
 *
...
/**
 * parseLegacy(value) - the behaviour of `require('url').parse(value)`, with
 * both of that call's deprecation warnings gone and nothing else changed.
 *
...
function parseLegacy(value) {
  ...
  // returns it untouched, before it validates the argument type at all. No
  // caller relies on this today; it is here because it is part of the
