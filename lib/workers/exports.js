var exportsQueue = require('../util/queues').exports()
  ...
  , legacyPathname = require('../util/legacyUrl').legacyPathname
...
function assetPathBasename(assetUrl) {
  return path.basename(legacyPathname(assetUrl));
}
...
function createExportArchive(userId, exportId, tempFile) {
  return new Promise(function(resolve, reject) {
    ...
    // `TypeError: <query>.stream is not a function`. The throw is caught by the Promise executor above,
    // surfaces in processBulkExport's catch, and its message is what gets persisted as the export's
    // errorMessage and emailed to the user. See docs/PRESERVED-QUIRKS.md section 3.24.
    ...
    stream.on('end', function() {
      Promise.all(trinketPromises)
        ...
        .then(function() {
          ...
          // immediately below, which is the SAME reject() the correctly-spelled
          // `archive.on('error', reject)` above already calls, so no outcome changes: a finalization
