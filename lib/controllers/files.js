...
/**
 * True for a payload value that is one of hapi's spooled multipart file parts.
 *
 * The three fields are checked TOGETHER because a client controls the field
 * names in a multipart body and can therefore submit an ordinary text part
 * called `upload` whose value is a string, or a JSON body carrying an object
 * with a `path` of its own. Only a part @hapi/subtext wrote to disk under
 * `multipart : { output : 'file' }` carries all three: a string `path`, a
 * numeric `bytes`, and the part's own `headers` object.
 *
 * @param {*} part a value taken from `request.payload`
 * @returns {boolean} true when the value is a spooled file part
 */
function isSpooledPart(part) {
  return !!part
    && typeof part === 'object'
...
/**
 * Deletes every spooled file part in a payload, ignoring every failure.
 *
 * WHY THIS EXISTS. `multipart : { output : 'file' }` means the parts are
 * already on disk before either handler below is entered, and the only path
 * that removes them is the successful one -- `lib/util/file.js`'s
 * `_fileToContainer` unlinks the temp file after the store call. Every arm that
 * answers WITHOUT uploading therefore leaves the bytes behind for the lifetime
 * of the host: the rejections added for QA finding W001-F04 would otherwise
 * institutionalise a temp-file leak an authenticated caller can drive at will
 * by repeating a malformed upload, and the avatar error arm already had one.
 *
 * Own enumerable values only, so a payload whose keys came from the wire cannot
 * reach anything inherited; `isSpooledPart` decides what is a file rather than
 * a path-like string, so nothing outside the spool directory can be named as a
 * target.
 *
 * Promise-form `fs` because this runs inside a hapi handler (AAP rule T-5).
 * Every failure is swallowed deliberately: the response owed to the client is
 * decided by the caller and must not turn into a 500 because a temp file was
 * already gone, and both callers are answering a rejection when they call this.
 * The one non-silent outcome is a log line, which is what makes a spool
 * directory filling up diagnosable.
 *
 * @param {Object} payload `request.payload`, or any falsy value
 * @returns {Promise<undefined>} resolved once every unlink has settled
 */
async function discardSpooledParts(payload) {
  var names, i, part;
  ...
  names = Object.keys(payload);

  for (i = 0; i < names.length; i++) {
    part = payload[names[i]];

    if (!isSpooledPart(part)) {
      continue;
    }
    ...
    try {
      ...
      // Swallowed by contract, recorded so the leak is visible if it recurs.
      console.log(err);
    }
  }
}
...
/*
 * The response-side policy served with the legacy image-download branch in
 * `download` below: no subresource of any kind may load, `sandbox` with no
 * allow-list puts the document in an opaque origin with scripting off, and it
 * may not be framed. Held in one named constant so the branch itself stays the
 * approved three-call chain plus two headers, and so the policy string has
 * exactly one definition to audit. Added under QA finding W002-I4; the branch's
 * own comment carries the reasoning and why nosniff beside it is not a
 * substitute.
 */
var LEGACY_IMAGE_CSP = "default-src 'none'; sandbox; frame-ancestors 'none'";
...
module.exports = {
  uploadAvatar : async function(request, h) {
    ...
    // The `upload : Joi.any().required()` this route used to declare, enforced
    // here instead. The declaration was removed because the route parser's
    // validation-failure funnel answers `request.fail(request.payload, ...)`,
    // which serves the rejected payload back at status 200 and so disclosed the
    // absolute path of the spooled part (QA finding W001-F04, config/routes.js).
    // Rejecting here keeps the 400 and loses the echo; the check is for a
    // spooled part rather than for mere presence because everything below it,
    // starting with `uploadUserAvatar`'s own `upload.headers['content-type']`,
    // reads a file part's fields and would otherwise throw a TypeError into the
    // route catch-all and answer 500.
    if (!isSpooledPart(request.payload && request.payload.upload)) {
      await discardSpooledParts(request.payload);
      return errors.badRequest('"upload" is required');
    }
    ...
    if (upload.err) {
      ...
      //
      // The spooled part is discarded first, and only here: this is the one arm
      // that answers without the upload utility having reached the unlink in
      // `_fileToContainer`, so a rejected avatar -- an unsupported image type,
      // or a digest that could not be read -- used to leave its temp file on
      // disk forever. The response itself is unchanged.
      await discardSpooledParts(request.payload);
  ...
  upload : async function(request, h) {
    ...
    // The two rules this route used to declare as
    // `{ type: Joi.string().valid('embed','download').optional(),
    //    upload: Joi.any().required() }`, enforced here instead and answering
    // the same 400 with the same message text.
    //
    // The declaration was removed because the route parser's
    // validation-failure funnel answers `request.fail(request.payload, ...)`,
    // and with `multipart : { output : 'file' }` on this route a file part IS
    // `{path, bytes, filename, headers}` -- so a rejected upload was answered
    // 200 with the absolute path of the spooled part in its body (QA finding
    // W001-F04, and the same echo fired whenever the file part was named
    // anything other than `upload`). The funnel itself is in
    // lib/util/routeParser.js, which this delivery does not change, so the
    // decision moves to where the response is built.
    //
    // No shipped client can reach either rejection: the material uploader at
    // public/js/courseEditor/controllers/materialControl.js:171-180 sends the
    // file part as `upload` and sends no `type` field at all, which is what the
    // defaulting below covers.
    var upload = request.payload && request.payload.upload;

    if (!isSpooledPart(upload)) {
      // A spooled part rather than mere presence: `uploadMaterialFile` and the
      // `contentType`/`fileName` reads below all take a file part's fields, so a
      // text part called `upload` would throw a TypeError into the route
      // catch-all and answer 500 instead of rejecting the request.
      await discardSpooledParts(request.payload);
      return errors.badRequest('"upload" is required');
    }

    if (typeof request.payload.type !== 'undefined'
        && request.payload.type !== 'embed'
        && request.payload.type !== 'download') {
      await discardSpooledParts(request.payload);
      return errors.badRequest('"type" must be one of [embed, download]');
    }
  ...
  download : async function(request, h) {
    ...
    // The promise boundary for the storage read is created HERE, at the
    // lifecycle method, rather than inside lib/util/file.js, which keeps its
    // callback interface (AAP rule T-3).
    //
    // `downloadMaterialFile` is still synchronous and still returns the
    // PassThrough immediately; what is awaited is its readiness signal, which
    // fires exactly once - `null` as soon as the read is known to be delivering
    // (the source's first byte, or the end of a zero-byte object) and the
    // store's own error if the read failed before any byte arrived. The
    // assignment to `stream` is therefore always complete before `resolve`
    // runs: the call returns before the callback can fire, because the callback
    // is only reachable from a later tick of the store's own request.
    //
    // WHY THIS IS AWAITED AT ALL. Baseline returned the stream without ever
    // consulting the read's outcome, so an absent or unreadable object produced
    // no response: the error surfaced as an unhandled 'error' event on the
    // PassThrough and the process exited 1 (measured from a single
    // unauthenticated request, on both trees), and once that was absorbed in
    // the utility the request answered with an empty reply instead - a stream
    // that would never deliver a byte. Neither outcome is a response a client
    // can act on, and R-b is unqualified about every route serving, so the read
    // outcome is turned into a status below. Awaiting it costs a successful
    // download nothing beyond the tick it already waits for: the signal arrives
    // with the first chunk rather than at the end of the object, so the
    // response still begins streaming before the body is complete and the time
    // to first byte for a large material is unchanged.
    var ready = new Promise(function(resolve) {
      stream = FileUtil.downloadMaterialFile(remoteFile, resolve);
    });

    // Unmoved, and still measuring exactly what it measured before: the
    // SYNCHRONOUS part of the read, from just before the call to just after it.
    // The await is deliberately below this block rather than above it - folding
    // the wait for the first byte into this window would silently change what
    // every existing debug record means, from the cost of issuing the read to
    // the store's time to first byte.
    ...
    var storageError = await ready;

    if (storageError) {
      // Returned rather than thrown, for the reason `uploadAvatar` above gives:
      // the route catch-all maps a THROWN Boom onto badImplementation, which
      // would flatten the 404 below into a 500.
      //
      // An absent object answers 404, which is the same status - and, since this
      // path begins /api/, the same JSON shape from app.js's onPreResponse -
      // that an absent File ROW already produces through this route's `file`
      // pre-handler. The two ways of asking for a file that is not there
      // therefore answer alike. `code` is the SDK v2 field both the real client
      // and the parity fixture set to 'NoSuchKey'; `statusCode` is checked as
      // well so a client that reports the absence only numerically is not read
      // as an internal fault.
      //
      // Every other read failure - a credentials error, an unreachable store, an
      // unreadable object file - is a fault on this side and answers 500 with
      // its own message, which is what the route catch-all produces for an
      // unexpected failure and what `uploadAvatar` above produces for a
      // non-Boom error. Nothing is logged that baseline did not log.
      return (storageError.code === 'NoSuchKey' || storageError.statusCode === 404)
        ? errors.notFound()
        : errors.badImplementation(storageError.message);
    }
    ...
    if (/^image/.test(request.pre.file.type)) {
      ...
      // Its approved content is the three-call chain below; the two protective
      // headers beside it are not part of that approval, and the last three
      // paragraphs of this comment record where they come from and why they
      // leave the approved contract intact.
      //
      ...
      //
      // The approved response is four fields, and all four are unchanged below:
      // 200, the file document's own mime as the content type, the file
      // document's own byte size as the body length, and NO Content-Disposition
      // -- carried, with the outcome change from an expected timeout to an
      // answered request, by the approved-deviation register in
      // test/parity/replay.js, which machine-checks exactly those. The three
      // calls that produce them are untouched, so the parity gate still approves
      // this scenario. Content-Disposition stays omitted because that omission is
      // the purpose of this branch, which renders an image inline rather than
      // downloading it.
      //
      // The two headers are added under QA finding W002-I4 and sit OUTSIDE those
      // four fields. Serving user-controlled bytes as active same-origin content
      // is not preserved behaviour for R-d to protect: it did not exist at
      // baseline, where this branch produced no response at all, so it was
      // CREATED by this deviation, and it was never analysed when the deviation
      // was approved. There is no baseline outcome here to preserve -- only an
      // exposure, measured anonymous and executing, to close.
      //
      // The two are not interchangeable and neither is redundant. nosniff only
      // stops a browser sniffing AWAY from the declared type, and the declared
      // type IS the document's own mime -- `text/html` for the legacy rows that
      // reach this branch -- so on its own it instructs the browser to honour
      // precisely the type that makes the bytes execute. The CSP is what closes
      // that: `default-src 'none'` forbids inline script and every subresource,
      // `sandbox` with no allow-list puts the document in an opaque origin, which
      // both disables scripting and severs it from the application's own storage,
      // and `frame-ancestors 'none'` keeps it out of the application's frames.
      // nosniff is kept beside it for the case the CSP does not speak to: a row
      // whose mime is inert being re-read as active content by a sniffing browser.
      //
      // Nothing here reads the object bytes. Classifying by signature,
      // substituting the content type, or falling back to an attachment
      // disposition would each breach one of the four approved fields above.
