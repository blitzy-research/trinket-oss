...
module.exports = {
  createCourse : async function(request, h) {
    ...
    try {
      ...
      // the measured pending fate is restored. lib/controllers/courses.js#create consumes this
      // route over server.inject and matches neither `.course` nor `.err`, so its own fall-through
      // stays reachable - and see the note there, because an inject of a pending route is itself
      // pending.
      return Pending.hang();
    }
    ...
    return course.addUser(request.user, ["course-owner"])
      .then(function() {
        // SEC-13 - see docs/PRESERVED-QUIRKS.md section 4.14. `Course.publicSpec` whitelists
        // `_owner`, and `setOwner` assigned the populated User DOCUMENT to it, so the success
        // responder's `serialize()` reached its nested-object branch. That branch tests
        // `hasOwnProperty('serialize')`, but mongoose installs `serialize` on the document
        // PROTOTYPE, so the test fails and the owner is JSON-cloned whole - shipping the owner's
        // bcrypt hash in this body and, through the server.inject in lib/controllers/courses.js#create,
        // in POST /courses too. The owner is therefore replaced with the very clone that branch would
        // have produced, scrubbed of credentials: every other key stays byte-identical, so the only
        // observable difference is the absence of values no client can legitimately read. The clone is
        // fresh, so `request.user` - which `setOwner` aliased here - is not mutated.
        //
        // The scrub is the centralized recursive one rather than a top-level `delete owner.password`,
        // because a Google-linked owner also carries a live OAuth bearer credential at
        // `profiles.google.token` (lib/models/user.js declares `profiles` as untyped Mixed, and
        // lib/controllers/auth.js#googleCallback writes the token), which the key-by-key delete left in
        // this response body and therefore in browser-side data, caches and any XSS-accessible state.
        if (course._owner && typeof course._owner.toJSON === 'function') {
          course._owner = Credentials.redact(JSON.parse(JSON.stringify(course._owner)));
        }
  ...
  copyCourse : async function(request, h) {
    ...
    ||  request.user.hasPermission("make-course-copy", "course", { id : request.pre.course.id })) {
      ...
      // Course#copy takes an error-first callback and returns nothing, so it needs this adapter.
      // No timeout by design: Course#copy leaves its inner chain unterminated, so a rejection from
      // the lesson copy never reaches the callback and the request stays open.
      try {
    ...
    if (request.user.hasPermission("manage-course-access", "course", { id : course.id })) {
      return User.findByLogin(request.payload.user)
        ...
        .then(function(result) {
          ...
          // so the fall-through returns a response that never settles instead of resolving
