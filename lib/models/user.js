...
//
// bcrypt 6.0.0 resolves a promise when no callback is supplied, and that promise form is what the
// async conversion requires here, completing the promise-native triad alongside genSalt and hash
// above. The optional-callback bridge is retained DELIBERATELY rather than dropped: the existing
// model spec drives this method as `comparePassword.call(user, 'foo', function(err, isMatch) { ... })`
// and existing test assertions are preserved, not rewritten. When a callback is supplied the bridge
// hands it exactly the pair bcrypt's own callback form produced and returns `undefined`, which is what
// bcrypt.compare itself returned once a callback was passed. The success argument is `undefined`, not
// `null`, because that is what bcrypt's own callback passed - measured on BOTH the base commit's
// bcrypt 5.1.1 and the installed 6.0.0: `err === undefined` is true and `err === null` is false on a
// match and on a mismatch alike. A failure gets `(err)` with an undefined second argument, again as
// bcrypt did. The three-argument form still works on 6.0.0 (measured), so this bridge is about the
// promise-native conversion the migration asks for, not about a removed API.
...
function exists(user, cb) {
  ...
  // Two-argument .then() rather than a trailing .catch(cb), for the reason recorded at
  // encryptPassword above and at findAdminList below: cb must be invoked exactly ONCE. Its
  // callers in lib/controllers/users.js answer the request from inside cb, so a second
  // invocation would attempt to answer an already-answered request. Baseline reached cb through
  // `this.model.find({ $or : or }, function (err, users) { if (err) return cb(err); ... })`,
  // one invocation per path, with a throw out of cb propagating rather than re-entering it.
  return this.model.find({ $or : or })
    .then(function(users) {
      ...
      return cb(err); // the identical mongoose error object, as the first argument
    });
}
