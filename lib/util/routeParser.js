 ...
 *
 * This is a presence test and nothing more: the switch is on when an argument is
 * exactly `-R` or `--routes`, or begins `-R=` or `--routes=`, and the value after
 * the `=` is not inspected. Direct execution then forces it on regardless, via
 * the `argv.R || executable` assignment below, which is what emits the route map
 * when this file is run with no argument at all.
 *
 * There is deliberately no negative form, and the option parser's own value
 * coercions are deliberately not reproduced. Measured against the optimist
 * declaration this replaced, `--routes=false` yielded the string `"false"` and
 * `-R=false` the string `"=false"` -- both truthy, so both turned the table ON:
 * no working off switch ever existed to preserve. Reproducing the rest of that
 * coercion lattice (`--routes=0` and `--routes=00` to the number 0, `-Rfalse`
 * and `--R` to true, last assignment winning for `-R --routes=false`) would mean
 * writing a new option parser in this file, which is the fifth kind of change
 * AAP rule R-a keeps out of this diff. What is contracted is narrower and is
 * held here in full: AAP sections 0.3.1 and 0.4.2 require exactly three
 * invocation forms to survive -- no argument, `-R`, and the `--routes` alias --
 * and the parity gate compares the table those three emit byte for byte.
 */
