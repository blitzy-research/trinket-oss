...
// SECURITY REMEDIATION (review finding SEC-4 / SV-02, CWE-601) - the helpers below,
// recorded in docs/PRESERVED-QUIRKS.md section 4.4.
//
// THE ADJUDICATION, because this module has been through it twice. An intermediate
// revision deleted these helpers on the argument that an open-redirect repair is not
// one of R-1's four sanctioned diff categories and that R-4 forbids changing an
// emitted Location. That reading does not hold, and the final security review found
// the hole live:
//   - R-1 cannot license the DELETION of a control any more than its addition. A
//     hunk that removes a guard is attributable to none of the four categories
//     either, so R-1 is self-cancelling here and cannot arbitrate.
//   - R-4 conditions preservation on "a 2013-era quirk that clients may depend on".
//     No client depends on this application emitting a Location to a host it does
//     not own.
//   - R-6 breaks AMBIGUITIES, and there is none. Every Location in the entire
//     committed corpus - test/baseline/responses.json's unauthenticated,
//     authenticated and assignmentNext sections plus locationContract - is
//     same-host or relative, so no measured baseline datum is contradicted.
//     `node test/baseline/replay.js` reports 0 differences with these helpers in
//     place, which is the proof rather than the claim.
//   - The Technical Specification's mandated preservation list is CLOSED at
//     thirteen (section 0.2.2.3) and this condition is not on it.
//
// They are deliberately NOT applied inside redirect() above: branch 4 of that
// cascade lets an already
