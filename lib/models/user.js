 ...
 *
 * Async conversion: the contract is promise-native. The base commit passed an error-first callback into
 * `this.model.find(query, cb)` and its two callers in lib/controllers/users.js each wrapped the result
 * back into a promise; the callback and both bridges are gone. The resolved shapes are byte-identical to
 * the ones those callbacks received - `{exists:false}` or `{exists:true, duplicates, users}` - and a
 * query failure rejects with the identical mongoose error object the callback took as its first
 * argument, which is what the callers' surrounding try/catch already handled.
 *
