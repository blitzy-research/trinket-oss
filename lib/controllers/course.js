...
module.exports = {
  ...
  getCourse : async function(request, h) {
    ...
    // here.
    //
    // SEAM-F167. This statement reads as sanitization and is not: measured
    // in-process, mongoose accepts the returned plain object on the `_owner`
    // `ref` path and HYDRATES it back into a User document, so the publicSpec
    // output loses its `id` virtual, gains a fresh `_id` and the schema
    // defaults `verified`, `source`, `roles`, `created` and `lastUpdated`, and
    // loses `password` and `__v`. It strips the password and keeps the email.
    // It is kept exactly as it is rather than removed: the document it rewrites
    // is `populate`d twice further down, so the hydration's side effects are
    // load-bearing for behaviour this change does not touch, and there is no
    // observable benefit to removing it now that the real projection happens at
    // the response boundary. See `tagSafeCourseProjection`.
    ...
    if (!request.query.outline) {
      // Projected for `_owner` only: no lesson is populated on this response,
      // and the course's own name must not be neutralized here because the
      // course editor loads it into an editable form that is submitted back -
      // the reasoning `tagSafeCourseProjection` carries in full.
      return request.success({ data : tagSafeCourseProjection(course, {}) });
    }
    ...
    return course.populate(lessonsPopulate)
      ...
      .then(function() {
        ...
        // byte-identical, and the non-outline response above asks for no name
        // neutralization at all because it carries no populated lesson - it
        // goes through the same wrapper only for the unconditional `_owner`
        // projection SEAM-F167 added. See `tagSafeCourseProjection` for why the
        // course name is not neutralized on this response.
      ...
      return await new Promise(function(resolve) {
        course.save(function(err, savedCourse) {
          if (err) {
            ...
            //
            // THIS IS THE BASE COMMIT'S OUTCOME, not a conversion artifact, and
            // it was re-examined because it reads like one. At `2f8712a` this
            // branch was `return reply({err: err, message: err.message})`, and
            // the shim's `reply` resolved its deferred only for a Boom, an
            // Error instance, or a subsequent `.code/.header/.redirect/.view`
            // call (lib/util/routeParser.js:360-408 at that commit). A plain
            // object argument with no chained call resolved nothing, and the
            // handler around it returned `undefined`, so the shim's
            // `result = await responsePromise` waited on a promise that nothing
            // would ever settle. Both trees therefore leave the request
            // unanswered, which is why this is preserved rather than fixed
            // under the never-settles rule that decided lib/controllers/
            // files.js's image branch.
            //
            // No HTTP measurement of it exists on either tree, and cannot: the
            // route's own payload schema (config/api_routes.js, `PUT /api/
            // courses/{courseId}/metadata`) admits only `name`, `description`,
            // `courseType` and `contentDefault`, every value it accepts is
            // accepted by the schema in lib/models/course.js, and the only save
            // failure reachable through it is the duplicate-key case the branch
            // above answers.
            return;
          }
  ...
  moveMaterial : async function(request, h) {
    ...
    if (request.user.hasPermission("manage-course-content", "course", { id : course.id })) {
      ...
      // `parent(payload.parent,pre.lesson)` resolves the payload's parent with
      // its own Lesson.findById, so a payload naming the material's CURRENT
      // lesson yields a second document instance of it. The two splices below
      // would then land on different instances of the same lesson, and the
      // `lesson.id !== parent.id` test would skip the second save - dropping
      // the material from the list instead of moving it. Collapsing the two
      // instances onto one keeps the payload-bearing and payload-less forms of
      // this route doing the same thing, which is what `request.pre.parent ||
      // lesson` above already intends. A parent naming a DIFFERENT lesson is
      // untouched: both documents are still spliced and both are still saved.
      ...
      // PRESERVED QUIRK, now reachable: on a CROSS-lesson move `oldParent`
      // reports the DESTINATION lesson, not the source. The first `.then`
      // below returns `parent.save()` when the two differ, so the second
      // `.then`'s own `lesson` parameter shadows the outer `lesson` with the
      // destination document, and `oldParent : lesson.id` reads it. Both
      // fields therefore carry the destination id, while `newParent`,
      // `oldIndex`, `newIndex` and - verified against both Lesson documents -
      // the persisted order are all correct. A SAME-lesson move is unaffected:
      // there the two ids are equal by construction, which is the shape
      // `moveLesson` above emits unconditionally and the shape the suite
      // asserts.
      //
      // Left exactly as baseline has it. The chain is byte-identical to
      // 2f8712a; what changed is that the route used to answer 500 before
      // reaching here (the two-argument `parent(...)` pre-handler), so the
      // field was never emitted. The gate that forced this route to serve
      // (AAP 0.9.2, via test/lib/api/course.js:217-227) asserts `oldIndex`,
      // `newIndex` and `newParent` only, no parity artifact records a 200 body
      // for this route - test/parity/corpus.json holds the 500 - and no client
      // reads `oldParent`: the identifier appears nowhere in public/js,
      // public/partials, lib/views or test/. So AAP R-d governs and the value
      // stays. Correcting it means binding the source id before either save
      // and reading that instead of the shadowed parameter, which is a
      // behaviour change to be taken deliberately, not folded into a
      // migration.
      return lesson.save()
        .then(function(lesson) {
      ...
      if (canUpdate) {
        return invitation.save()
          ...
          .catch(function(err) {
            ...
            // {"err":{},"message":"..."}.
            //
            // And 200 `{}` is what the base commit answered too, which is the
            // reason for the empty object rather than a preference. At `2f8712a`
            // this branch was `return reply({err: err, message: err.message})`;
            // for a plain-object argument the shim's `reply` built a response,
            // resolved nothing, and returned its chainable BUILDER, so the
            // value this returned chain carried to hapi was that builder - an
            // object whose every own property is a function. hapi wrapped it
            // and JSON.stringify dropped all six, giving 200
            // application/json with a two-byte `{}` body. Returning `{}`
            // directly reproduces that byte for byte without depending on the
...
/**
 * The only fields of a populated course owner that leave this controller.
 *
 * SEAM-F167. Measured against every consumer in the tree: `username` is read
 * by `public/js/classPage/app.js:291,296` and `name` by
 * `public/partials/course_editor.html:11`, and nothing else reads any field of
 * a populated `_owner` at all. The order is the order the keys are emitted in,
 * which keeps the projected object stable across requests.
 */
var OWNER_PUBLIC_FIELDS = ['name', 'username'];

/**
 * Reduces a populated `_owner` on an already-serialized course to
 * `OWNER_PUBLIC_FIELDS`, in place.
 *
 * SEAM-F167. Called unconditionally by `tagSafeCourseProjection`, whose doc
 * comment carries the leak, the two-part root cause and the reasoning for the
 * whitelist. This function is only the mechanism, and it has one job beyond
 * copying: recognising the value it must NOT touch.
 *
 * `Model.serialize` emits an unpopulated ObjectId `_owner` as a STRING - it
 * falls through to `JSON.parse(JSON.stringify(objectId))`, which is the id's
 * hex form - and that string is the shape every course response carries today.
 * A non-object, an array and a null therefore return unchanged, so only the
 * `with=_owner` population path is affected and every other response stays
 * byte-identical.
 *
 * @param   {Object} serialized  a serialized course, mutated in place
 * @returns {Object}             the same object, for call-site convenience
 */
function projectOwner(serialized) {
  var owner = serialized ? serialized._owner : null
    , projected;
  ...
  OWNER_PUBLIC_FIELDS.forEach(function(field) {
    // Only a key that is actually present is copied, so an owner document
    // carrying no `name` does not acquire an explicit `null` one - the same
    // rule the name neutralization above follows.
    if (Object.prototype.hasOwnProperty.call(owner, field)) {
      projected[field] = owner[field];
    }
  });

  serialized._owner = projected;
 ...
 * `courseName`.
 *
 * SEAM-F167. The same wrapper is where a POPULATED `_owner` is bounded, and
 * unlike the name fields above that projection is UNCONDITIONAL - it is not
 * asked for through `fields` and it does not consult the caller's identity.
 * What leaked: `GET /api/courses/{courseId}?with=_owner` is reachable with no
 * session (the route declares no `auth`, so it inherits `mode: 'try'`), and its
 * `populate(pre.course, query.with)` pre-handler is what the class view's own
 * XHR asks for, so an anonymous caller holding only the course id received the
 * owner's `email` together with `fullname`, `name`, `username`, `avatar`,
 * `settings`, `source` and `verified`.
 *
 * The root cause is in two halves, and neither is where it looks.
 * `getCourse`'s `course._owner = course._owner.serialize()` does not sanitize:
 * mongoose accepts the returned plain object on a `ref` path and hydrates it
 * back into a User document, so `serialize()`'s `id` virtual is dropped, a
 * fresh `_id` and the schema defaults `verified`, `source`, `roles`, `created`
 * and `lastUpdated` appear, and `password` and `__v` are gone - it strips the
 * password and keeps the email. Then `Model.serialize`
 * (lib/models/model.js:77-86) walks `Course.publicSpec`, which includes
 * `_owner`, and tests `hasOwnProperty('serialize')` on it - false, because
 * `serialize` is a schema method on the prototype - so it falls through to
 * `JSON.parse(JSON.stringify(this[key]))` and emits the whole document. The
 * projection therefore happens HERE, at the response boundary, where both
 * halves have already run and the value is a plain object this function owns.
 *
 * The whitelist is `{name, username}` because that is what the tree reads:
 * `public/js/classPage/app.js:291,296` builds an edit and a dashboard location
 * from `course._owner.username`, and `public/partials/course_editor.html:11`
 * renders `course._owner.name`. Nothing in `public/js`, `public/partials` or
 * `lib/views` reads `_owner.email` or `_owner.id`. `username` discloses nothing
 * new either, because `ownerSlug` is already in `Course.publicSpec` and carries
 * the same value. One shape therefore serves every reader, which is why this is
 * not gated on identity: a second, wider shape for members would add a branch
 * that no client needs and that could only leak.
 *
 * An UNPOPULATED `_owner` serializes to a STRING - `Model.serialize`'s
 * `JSON.stringify` of an ObjectId - and is left exactly as it is, so every
 * response that does not ask for `with=_owner` is byte-identical, including the
 * bare id string that `test/parity/corpus.json` records for
 * `GET /api/courses/{courseId}`. Only keys actually present are copied, so an
 * owner with no `name` does not acquire a `null` one.
 *
