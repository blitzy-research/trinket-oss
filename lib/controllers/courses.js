...
module.exports = {
  ...
  copy : async function(request, h) {
    ...
    // `lib/models/model.js`'s model-level 'error' listener - added by this
    // migration for a different edge - already removed the exit, leaving the
    // request hung: measured 000 at 20.002 s with the server still serving.
    // R-b does not permit either outcome, and preserving the exit is not
    // available to preserve, so the two failure paths below answer.
    //
