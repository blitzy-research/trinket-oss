...
Runs the application on Node 22 LTS against the real `@hapi/hapi` 21.x API. Behaviour was preserved deliberately, and most of that preservation is gate-backed rather than intended: the route surface is compared entry by entry over all 233 routes, the validation outcomes over all 102 targets, the storage and worker contracts case by case — each measured on the delivered tree after `npm ci` from the committed lockfile, because what the gates measure is the installed graph. **Four of the five `npm run verify:*` gates exit 0**: `verify:routes` (the HTTP surface identical across all 233 entries), `verify:joi` (zero parity differences over 102 targets, 306 cases, 462 outcomes and 15 678 fields), `verify:storage` (35 of 35 cases) and `verify:worker` (VERDICT PASS, 109 of 109 checks over 7 real jobs on `bull` 4.16.5, 0 notices). Boot under `node --pending-deprecation --trace-deprecation` emits **no warning or deprecation line at all** and `GET /` answers 200, where the baseline emitted four warning classes; `npm audit --omit=dev` reports 0 critical, 1 high and 6 moderate, the high being the retained `marked` fork named under **Behaviour**. The rest is stated as what it is rather than folded into that: `npm run verify:corpus` **runs** on both cookie passes and does not qualify as a gate run, for a structural reason rather than a comparison result, with 380 of 391 driven scenarios matching in the non-secure pass and 10 differing; per-path conversion closure is complete at 154 of 154, with 29 site-level rows open because closing them is a measurement rather than a reading; the changed-error-edge inventory closes 295 of its 372 rows; the container and image results were measured at an earlier commit whose image inputs have since changed, so they are recorded rather than re-authenticated here; the suite registers and executes its full 130-case contract, of which **103 pass and 27 fail** (the run reports 28 failures: those 27 cases plus the suite-total gate assertion itself); and the delivered tree changes 15 paths the frozen plan does not declare. **Not yet proven** at the end of this entry records the gates one by one, **Delivery** carries the path census, and the two approved deviations from preservation are named under **Behaviour**.
...
- **`supertest` stays at 0.8.3**, and the move to `^7.1.4` that an interim delivery made is **withdrawn**. The argument for it was real as a mechanism — `superagent` 0.8 emits `Content-Disposition: attachment` on multipart parts, which RFC 7578 §4.2 forbids, and hapi 21 answers 400 to such a part — but unreachable in this tree: the shipped routes never enable `payload.multipart`, so hapi answers **415** before any part header is read, and the suite-scoped switch that used to turn parsing on is withdrawn too. **Measured**: the two upload cases answer 415 and the two downloads behind them 404 with either agent installed, so the major changed no case's outcome and an unforced harness bump is exactly what R-a excludes. AAP §0.5.1.6 holds this package unchanged and it is now unchanged; the five development-only advisories the bump had removed are back with it, outside the `--omit=dev` gate, and the whole episode is recorded in [docs/dependency-inventory.md](docs/dependency-inventory.md) §5 and [docs/deferred-dependencies.md](docs/deferred-dependencies.md) §2.11.
...
under **Security** above. What remains open is the corpus gate's own qualification and the 27 failing
