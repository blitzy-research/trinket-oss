...
module.exports = {
  ...
  grantRole : async function(request, h) {
    ...
    return user.grant(request.payload.role, "site")
      ...
      .then(function(user) {
        // PRESERVED QUIRK (docs/PRESERVED-QUIRKS.md section 4.14). The `JSON.parse(JSON.stringify(user))`
        // round-trip is the base commit's payload shape and is reproduced verbatim: it clones the whole
        // User document rather than projecting `publicSpec`, so this response ships every persisted key
        // of another user - including the bcrypt `password` hash and, for a Google-linked account, the
        // live OAuth bearer credential at `profiles.google.token` that lib/controllers/auth.js writes
        // into the untyped Mixed `profiles` object.
        //
        // It is deliberately NOT scrubbed. R-4 freezes client-visible payload shapes and R-6 makes the
        // base commit the tie-breaker, and no exception in the Technical Specification authorizes a
        // security change here; the condition is catalogued instead. Nothing in the repository reads
        // this response - the admin role editor at lib/views/admin/index.html posts to
        // /api/admin/user/{id} and reads only `result.success`.
        return h.respond({
          success : true,
...
async function userSearch(q) {
  ...
  // PRESERVED QUIRK (docs/PRESERVED-QUIRKS.md section 4.14). This round-trip is the base commit's shape
  // and is reproduced verbatim, including its consequence: it flattens the User document to a plain
  // object BEFORE the success responder runs, so `ObjectUtils.serialize`'s prototype-safe
  // `typeof(json.serialize)` test finds nothing to project and the whole document reaches the client.
  // That is how the searched user's bcrypt hash - and a Google-linked account's OAuth bearer credential
  // at `profiles.google.token` - arrives in the rendered admin page, which dumps this object wholesale
  // through `{{ data | json("pretty") | safe }}` in lib/views/admin/includes/users.html. The template's
  // named reads are `_id`, `avatar`, `coursesOwned`, `email`, `name`, `roles`, `trinketsOwned` and
  // `username`, and `roleSearch` below answers with real documents rather than a clone.
  //
  // No scrub is applied: R-4 freezes the payload shape, R-6 makes the base commit the tie-breaker, and
  // the condition is catalogued in the preserved-quirks document rather than fixed on this branch.
