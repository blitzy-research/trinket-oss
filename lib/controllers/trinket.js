...
function legacyReply(value, h) {
  ...
  // the builder was returned unresolved and hapi serialised the builder itself.
  return h.response({});
}
...
module.exports = {
  ...
  draft : async function(request, h) {
    ...
    if (request.payload.zipCode) {
      ...
      // `return` added, which T-1 requires: the chain's value is this branch's
      // response. It also changes what happens on a malformed zipCode, and that is
      // unavoidable rather than discretionary. Baseline: loadAsync rejects, the
      // first onRejected returns a response OBJECT, the next onFulfilled receives
      // it as `code`, and JSON.parse("[object Object]") throws inside a chain with
      // no downstream rejection handler -- an unhandled rejection which, under
      // Node 22's default --unhandled-rejections=throw, TERMINATED THE SERVER
      // PROCESS. Measured: no response, and every other route dead with it. There
      // is no way to satisfy T-1 and still crash: once the chain is returned, its
      // rejection is the handler's rejection and the preserved catch-all maps it to
      // a 500. So this branch now answers 500 instead of killing the process.
  ...
  autosave : async function(request, h) {
    ...
    if (request.user.id.toString() === request.pre.trinket._creator.toString()) {
      ...
      if (request.payload.zipCode) {
        ...
        // `return` added per T-1, with the same unavoidable consequence recorded on
        // trinket.draft above: baseline let JSON.parse throw into an unhandled
        // rejection and TERMINATED THE SERVER PROCESS (measured), because the Boom
        // produced by the first onRejected arrived here as `code`. Returning the
        // chain routes that throw to the preserved catch-all instead, so the branch
        // answers 500 and the process survives.
        return zip.loadAsync(request.payload.zipCode, { base64: true })
          .then(function(content) {
            return content.file("zipCode").async("string");
          }, function(err) {
