...
var util             = require('util'),
    ...
    // Only for isSecretField(). lib/http/responseContract.js#reject logs the `err` string built
    // below, and Joi embeds the OFFENDING VALUE in its pattern/min/max messages, so a rejected
    // password would reach the log in cleartext. The definition of a secret-bearing name is taken
    // from the responder rather than duplicated, so the two cannot drift. LOG ONLY: the map that is
    // flashed and re-rendered is `validationErrors`, which is untouched.
...
function validate(request, h, validation, language, reject) {
  ...
  if (validation) {
    ...
    if (validationErrors) {
      ...
      // `request.payload` is still handed over WHOLE: it is flashed as 'failure' and re-rendered,
      // which is the frozen behavior, and the responder redacts it only on the way to the log. The
      // second argument is log-only, so it is pre-scrubbed here.
