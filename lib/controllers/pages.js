...
module.exports = {
  ...
  home: async function(request, h) {
    ...
    return Trinket.findRecentByOwner(request.user._id)
      ...
      // "<inspected error> undefined" and treats the error object as the response payload. This
      // route's `html : 'home.html'` hoists into `success.html`, which the rejection path never
      // reads, and it declares no `fail`, so both of that path's redirect and view branches are
      // skipped and h.response() receives the Error itself, which hapi refuses: the wire result is a
      // scrubbed HTTP 500, never a 4xx. Keep it byte-for-byte: do NOT wrap this in
