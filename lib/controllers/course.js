...
module.exports = {
  createCourse : async function(request, h) {
    ...
    return course.addUser(request.user, ["course-owner"])
      .then(function() {
        // SEC-13, extended by review finding M6 - see docs/PRESERVED-QUIRKS.md section 4.14.
        // `Course.publicSpec` whitelists `_owner`, and `setOwner` assigned the populated User DOCUMENT
        // to it, so the success responder's `serialize()` reached its nested-object branch. That branch
        // tests `hasOwnProperty('serialize')`, but mongoose installs `serialize` on the document
        // PROTOTYPE, so the test fails and the owner is JSON-cloned whole - shipping the owner's bcrypt
        // hash in this body and, through the server.inject in `lib/controllers/courses.js#create`, in POST /courses too. The
        // owner is therefore replaced with the very clone that branch would have produced, scrubbed of
        // credentials: every other key stays byte-identical, so the only observable difference is the
        // absence of values no client can legitimately read. The clone is fresh, so `request.user` -
        // which `setOwner` aliased here - is not mutated.
        //
        ...
        // `profiles.google.token` (lib/models/user.js:L18 declares `profiles` as untyped Mixed;
        // lib/controllers/auth.js:L232/L253 write the token), which the key-by-key delete left in this
        // response body and therefore in browser-side data, caches and any XSS-accessible state.
