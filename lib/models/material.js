...
// Returns a promise that settles from the final material.save() - resolved with the saved document,
// rejected with the save error - or, when the chain that leads to that save fails, rejected with the
// SILENT-OUTCOME SENTINEL from lib/models/model.js#silentOutcome. A rejection from Trinket.findById,
// trinket.copy or parser.parse therefore answers NO RESPONSE: the sentinel travels up through
// lib/models/lesson.js#copy and lib/models/course.js#copy to the two controllers, which answer it with
// `h.abandon`, and the rejection stays owned rather than pending.
  ...
  if (this.type === 'assignment') {
    Trinket.findById(this.trinket.trinketId)
      ...
      .then(function(trinketCopy) {
        ...
        // The final save is the only settle path on the success side: its document resolves and its
        // error rejects, unwrapped.
