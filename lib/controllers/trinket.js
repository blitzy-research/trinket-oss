...
// The legacy-pathname derivation is SHARED with lib/workers/exports.js rather than
// duplicated per call site, because both consumers derive PERSISTED, CLIENT-VISIBLE values
// from it: here, the asset filenames written into a downloaded or a posted zip archive;
// there, the S3 object key an asset is read from and the entry name it is appended under.
// Both are storage-format surfaces frozen by TR6. lib/util/legacyUrl.js carries the full
// derivation, both genuine parser disagreements, and the differential-verification record -
// including the explicit note that NON-STRING inputs are outside the verified claim (the
// legacy parser rejected them with ERR_INVALID_ARG_TYPE while URL.parse() stringifies
// first) and are unreachable by construction rather than neutralised.
var legacyPathname = require('../util/legacyUrl').legacyPathname;
...
module.exports = {
  ...
  updateMetrics : async function(request, h) {
    ...
    if (!metric) {
      ...
      //
      // Model.findById(id, callback) hands back the mongoose Query object itself, so this
      // statement returns a thenable that has ALREADY been executed by the callback form.
      // routeParser awaits whatever the handler returns, which re-executes that Query and
      // makes mongoose throw 'Query was already executed', so this branch answers HTTP 500
      // with the message scrubbed and the inner request.success never reaches the wire. Keep
      // the statement and keep this handler non-async: do not "repair" it into a 200.
  ...
  downloadFile : async function(request, h) {
    ...
    return Trinket.findById(request.params.shortCode)
      .then(function(trinket) {
        ...
        if (file) {
          // legacyPathname() reproduces the deprecated legacy URL parser's pathname. It is the shared
          // adapter in lib/util/legacyUrl.js, required at the head of this file; see
  ...
  downloadPostedZip : async function(request, h) {
    ...
    assets.forEach(function(asset) {
      ...
      if (/^data:/.test(asset.url)) {
        ...
        // legacyPathname() reproduces the deprecated legacy URL parser's pathname - the shared adapter
        // in lib/util/legacyUrl.js, required at the head of this file (docs/PRESERVED-QUIRKS.md section
        // 3.26). This call site is UNGUARDED - only `if (!asset.url) return;` and the /^data:/ test
        // precede it - so the helper must reproduce that parser's pathname exactly, including for the
        // relative, protocol-less and empty input that neither WHATWG entry point accepts, and must
...
function downloadZip(request, h) {
  ...
  trinket.assets.forEach(function(asset) {
    // legacyPathname() reproduces the deprecated legacy URL parser's pathname - the shared adapter in
    // lib/util/legacyUrl.js, required at the head of this file (docs/PRESERVED-QUIRKS.md section 3.26).
    // This call site is UNGUARDED, so the helper must reproduce that parser's tolerance of relative,
    // protocol-less and empty input rather than the rejection both WHATWG entry points give them.
    assetUrl  = legacyPathname(asset.url);
    assetFile = path.basename(assetUrl);
