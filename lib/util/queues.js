...
// These nine names and their order are frozen. Each resolves to a NoOpQueue that silently
// swallows every add(), which is what keeps Redis genuinely optional for a single-node
...
function createQueue(name) {
  ...
  if (redisEnabled) {
    // This require stays lazy on purpose: the application must still start when bull cannot
    // resolve or when no live queue is ever created.
...
// db.redis.bullqueues is a one-element list, so 'exports' is the only live queue and the nine
// disabled names above stay reachable only through createQueue, never as an exported getter.
