...
function copy(user, cb) {
  ...
  //
  // PRESERVED QUIRK - see docs/PRESERVED-QUIRKS.md. The two-argument .then reproduces the `if (err)`
  // guard exactly: the rejection handler runs instead of the body, never after it. A trailing .catch
  // would instead divert the deliberately unterminated inner chain below into cb. The duplicate-key
  // error this first save exists to catch must reach cb unwrapped, so the handler forwards it as-is,
  // with the single-argument cb(err) and the logging call both retained verbatim.
  course.save().then(function(doc) {
    ...
    Promise.all(lessonPromises)
      ...
      .then(function(ids) {
        ...
        // PRESERVED QUIRK - see docs/PRESERVED-QUIRKS.md. This chain is deliberately left unterminated: a
        // rejection from Lesson.findById or lesson.copy never reaches cb. No `return` here, and the
        // two-argument .then keeps success and failure disjoint so a throw from cb is not fed back into cb.
        // The logging call below is deliberately retained - removing logging is not a sanctioned change.
        course.save().then(function(doc) {
          return cb(null, doc);
        }, function(err) {
          err && console.log(err);
