...
// qs.stringify: a field whose value is `undefined` is OMITTED ENTIRELY, and every surviving key and
// value is percent-encoded with encodeURIComponent - so a space becomes %20, never +.
// Both properties are load-bearing and neither is what fetch does with a URLSearchParams body:
//   * `new URLSearchParams({ client_secret : undefined })` serializes the literal string "undefined",
//     which under partial configuration - config.app.auth.google.clientID set while .clientSecret and
//     .callbackURL are not - would send Google two junk field values instead of omitting them;
...
// Measured byte-identical to request@2.88.2 + qs@6.5.5 for this file's field maps, under both full and
...
module.exports = {
  ...
  google : async function(request, h) {
    ...
    // same-origin root-relative destination is persisted. Every in-application
    // path is returned unchanged, so '/auth/google?next=/home' behaves exactly as
    // it did.
    var next = Redirect.internalDestination(request.query.next);
    if (next) {
  ...
  googleCallback : async function(request, h) {
    ...
    try {
      ...
      // L189 above and any '/…' the visitor asked for still win over the '/home'
      // fallback; anything else falls back to '/home', which is exactly what an
      // absent `next` already did.
      var redirectTo = Redirect.internalDestination(request.yar.get('next')) || '/home';
