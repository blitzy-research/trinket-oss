var config       = require('config'),
    ...
    // SECURITY REMEDIATION (review finding SEC-4 / SV-02) - the same-origin destination
...
module.exports = {
  ...
  create : async function(request, h) {
    ...
    // SECURITY REMEDIATION (review finding SEC-4 / SV-02, CWE-601), recorded in
    ...
    // Only a same-origin destination is honoured; Redirect.internalDestination
    // returns both an in-application path and an absolute URL on one of this
    // application's own origins unchanged, so '/courses/x' and the absolute
    // window.location.href the frozen assignment UI sends (review finding P3-1)
    // both redirect exactly as they did at the base commit, while an off-origin or
    // scheme-relative value takes the no-redirect branch that an absent `next`
    // already took.
  ...
  login : async function(request, h) {
    ...
    // destination unchanged - both '/courses/x' and the absolute
    // window.location.href the frozen assignment UI sends, which is the flow review
    // finding P3-1 restored and test/baseline/responses.json#assignmentNext measures
    // - and answers null for anything that would leave the origin, which routes the
    // handler into the SAME `else` branch an absent `next` already took (the
    // declarative success.redirect '/home').
