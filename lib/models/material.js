  ...
  if (this.type === 'assignment') {
    Trinket.findById(this.trinket.trinketId)
      ...
      .then(function(trinketCopy) {
        ...
        // Deliberately unterminated: a rejection from Trinket.findById or trinket.copy never reaches
        // cb. No `return`, and the two-argument .then keeps a throw from cb out of cb.
    ...
    parser.parse(materialData.content, user)
      .then(function(parsedContent) {
        ...
        // Deliberately unterminated: a rejection from parser.parse never reaches cb. No `return`, and
        // the two-argument .then keeps a throw from cb out of cb.
        material.save().then(function(doc) {
          cb(null, doc);
        }, function(err) {
          cb(err);
