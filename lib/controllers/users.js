var config       = require('config'),
    ...
    // query string on `.path` - which :605 relies on. See :588 and :605.
    ...
    // in assetUploadFromURL. Readable.fromWeb() adapts fetch's web stream to the
    // Node stream that pipes into fs.createWriteStream. See :583.
...
module.exports = {
  ...
  // The `request` package's streaming GET is replaced here by native fetch. Every
  // observable behaviour of the original pipeline was measured against a fixture
  // server covering 200, a query-bearing 200, a 301->302->200 chain, 404, 500, a
  // mid-stream socket failure and a refused connection, and this implementation
  // reproduces all seven identically - event order, captured content-type, bytes
  // written, and whether the upload runs:
  //
  //   * redirects are followed, and the content-type comes from the FINAL
  //     response only, never an intermediate 3xx;
  ...
  assetUploadFromURL : async function(request, h) {
    ...
    return await new Promise(function(resolve) {
      // Reproduces the replaced package's transport gate: it accepted only
      // http/https and raised on everything else before any listener was
      // attached, so ftp:, file:, data:, mailto:, javascript:, ws:, gopher: and
      // blob: URLs have always left this route unsettled. fetch would happily
      // resolve a data: URL, so without this the hang would become a successful
      // upload - a behaviour change. Checked before the write stream is opened,
      // because the original threw before it ever created the temp file.
      if (requestUrl.protocol !== 'http:' && requestUrl.protocol !== 'https:') {
        console.log('on error:', new Error('Invalid protocol: ' + requestUrl.protocol));
        return;
      }
      ...
      fetch(request.payload.url).then(function(response) {
        contentType = response.headers.get('content-type');

        var body = response.body ? Readable.fromWeb(response.body) : Readable.from([]);
        ...
        body.on('end', startUpload);

        body.pipe(writeStream);
      }, function(err) {
        // A transport failure before any response LOGS ONLY. This must not
        // reject or resolve: the original never called back, so nothing settles
        // and the request hangs. A try/catch here would turn that into a 500.
        console.log('on error:', err);
      });
    });
  },
