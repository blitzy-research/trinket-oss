...
function parseRoutes(routeConfigs) {
  ...
  routeConfigs.forEach(function(route, index) {
    ...
    // that one declaration rather than a per-request copy.
    //
    // That is load bearing rather than incidental. lib/http/responseContract.js#reject
    // interpolates `fail.redirect` and writes the RESULT BACK into this same object, exactly as
    // the base commit did, so one request's interpolated value is visible to every request that
    // follows it on the same route. The write-back is preserved deliberately (R-4, and AAP
    // 0.4.1.1 lists it among the behaviors to relocate unchanged); a revision that interpolated
    // into a request-local instead was reverted. See docs/PRESERVED-QUIRKS.md section 4.4.
