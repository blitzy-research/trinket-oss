...
module.exports = {
  ...
  updateUser : async function(request, h) {
    ...
    // Each flattened lookup keeps its own try/catch so the failure still reaches request.fail
    // rather than the centralized error map. request.fail ends in h.response(json) and hapi
    // refuses to wrap an Error, so a raw Error argument makes the responder raise and the
    // request answers a scrubbed 500, while a plain object answers 200.
    try {
  ...
  grantRole : async function(request, h) {
    ...
    // handed to request.fail instead of escaping to the centralized error map.
    return user.grant(request.payload.role, "site")
      .then(function(user) {
        if (request.payload.role === "trinket-teacher") {
          ...
          // only, which the chain's .catch hands to request.fail. Adding the require would flip
