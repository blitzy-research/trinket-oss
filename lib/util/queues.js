...
// PRESERVED QUIRK: these nine names and their order are frozen. Each resolves to a NoOpQueue that
// silently swallows every add(), which is what keeps Redis genuinely optional for a single-node
// deployment. See docs/PRESERVED-QUIRKS.md section 3.11.
...
function createQueue(name) {
  ...
  if (redisEnabled) {
    // PRESERVED QUIRK: this require stays lazy on purpose - it is load-bearing for boot, because the
    // application must still start when bull cannot resolve or when no live queue is ever created.
...
// PRESERVED QUIRK: db.redis.bullqueues is a one-element list, so 'exports' is the only live queue and
// no other getter is exposed here - the nine disabled names above stay reachable only through
// createQueue, never as an exported getter. See docs/PRESERVED-QUIRKS.md section 3.11.
